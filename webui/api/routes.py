...
# ── Pantheon paths ─────────────────────────────────────────────────────────
PANTHEON_DIR = os.path.expanduser("~/pantheon")
EXPORTS_DIR = os.path.join(PANTHEON_DIR, "god-exports")
...
_WORK_ITEMS_STATE_FILE = STATE_DIR / "work-items.json"
_WORK_ITEMS_STATE_LOCK = threading.Lock()
_WORK_ITEM_STATE_RETENTION_SECONDS = 2 * 24 * 60 * 60
_SKILL_TRASH_LOCK = threading.Lock()
_SKILL_TRASH_DIRNAME = "skills-trash"
...
def _load_work_item_state() -> dict:
    try:
        raw = json.loads(_WORK_ITEMS_STATE_FILE.read_text(encoding="utf-8"))
    except FileNotFoundError:
        return {"items": {}}
    except Exception:
        logger.warning("Failed to read work item state from %s", _WORK_ITEMS_STATE_FILE, exc_info=True)
        return {"items": {}}
...
def _save_work_item_state(state: dict) -> None:
    _WORK_ITEMS_STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp = _WORK_ITEMS_STATE_FILE.with_suffix(_WORK_ITEMS_STATE_FILE.suffix + ".tmp")
    tmp.write_text(json.dumps(state, indent=2, sort_keys=True), encoding="utf-8")
    tmp.replace(_WORK_ITEMS_STATE_FILE)


def _prune_work_item_state(state: dict, now: float | None = None) -> bool:
    ...
    if not isinstance(items, dict):
        ...
        return True
    changed = False
    cutoff = now - _WORK_ITEM_STATE_RETENTION_SECONDS
    for item_id, item_state in list(items.items()):
        if not isinstance(item_state, dict):
            items.pop(item_id, None)
            changed = True
            continue
        updated = item_state.get("updated_at") or item_state.get("created_at") or 0
        try:
...
def _session_for_stream_id(stream_id: str):
    with LOCK:
...
def _work_items_payload() -> dict:
    with _WORK_ITEMS_STATE_LOCK:
        state = _load_work_item_state()
        changed = _prune_work_item_state(state)
        stored = state.get("items", {}) if isinstance(state.get("items"), dict) else {}
        if changed:
    ...
    with STREAMS_LOCK:
        ...
        tool_calls_by_stream = {
            sid: copy.deepcopy(STREAM_LIVE_TOOL_CALLS.get(sid, [])) for sid in stream_ids
        }
        cancelled = {sid for sid in stream_ids if CANCEL_FLAGS.get(sid)}

    for stream_id in stream_ids:
        item_id = f"stream:{stream_id}"
        stored_state = stored.get(item_id, {}) if isinstance(stored, dict) else {}
        if isinstance(stored_state, dict) and stored_state.get("status") in {"dismissed", "done"}:
            continue
        session = _session_for_stream_id(stream_id)
        title = "Agent turn"
        description = "Running agent stream"
        session_id = ""
        if session is not None:
            session_id = getattr(session, "session_id", "") or ""
            pending = str(getattr(session, "pending_user_message", "") or "").strip()
            title = str(getattr(session, "title", "") or "Agent turn").strip() or title
            if pending:
                description = pending[:160]
        tool_calls = tool_calls_by_stream.get(stream_id) or []
        items.append({
            "id": item_id,
            "kind": "Agent",
            "type": "agent_stream",
            "status": "cancelling" if stream_id in cancelled else "running",
            "title": title,
            ...
            "updated_at": now,
            "updated_at_display": _format_timestamp(now),
            "dismissible": True,
        })
...
def _handle_work_items_get(handler):
    return j(handler, _work_items_payload())


def _handle_work_items_update(handler, body):
    action = str(body.get("action") or body.get("status") or "").strip().lower()
    raw_id = body.get("id") or body.get("item_id")
    stream_id = str(body.get("stream_id") or body.get("streamId") or "").strip()
    item_id = str(raw_id or (f"stream:{stream_id}" if stream_id else "")).strip()
    if action not in {"dismiss", "dismissed", "done", "complete", "completed"}:
        return bad(handler, "action must be dismiss or done")
    if not item_id:
        return bad(handler, "id or stream_id required")
    status = "dismissed" if action in {"dismiss", "dismissed"} else "done"
    now = time.time()
    with _WORK_ITEMS_STATE_LOCK:
        state = _load_work_item_state()
        _prune_work_item_state(state, now)
        items = state.setdefault("items", {})
        if not isinstance(items, dict):
            items = {}
            state["items"] = items
        items[item_id] = {
            ...
            "detail": str(body.get("detail") or body.get("description") or "")[:500],
        }
        _save_work_item_state(state)
    payload = _work_items_payload()
    payload.update({"id": item_id, "stream_id": stream_id or None, "status": status})
    return j(handler, payload)


def _toolset_label(toolset: str) -> str:
    return str(toolset).replace("mcp-", "MCP: ").replace("_", " ").replace("-", " ").title()


def _handle_toolsets_list(handler):
    try:
        from api.config import get_config as _get_cfg
        toolsets = list(_resolve_cli_toolsets(_get_cfg()) or [])
    except Exception:
        toolsets = list(_resolve_cli_toolsets() or [])
    normalized = []
    seen = set()
    for raw in toolsets:
        ...
        if not name or name in seen:
            continue
        seen.add(name)
        normalized.append({"id": name, "name": name, "label": _toolset_label(name)})
    normalized.sort(key=lambda row: row["label"].lower())
    return j(handler, {"ok": True, "success": True, "toolsets": normalized})


def _delegation_config() -> dict:
    from api.config import get_config as _get_cfg, reload_config as _reload_cfg

    _reload_cfg()
    cfg = _get_cfg()
    raw = cfg.get("delegation", {}) if isinstance(cfg, dict) else {}
    return raw if isinstance(raw, dict) else {}


def _delegation_info_payload() -> dict:
    cfg = _delegation_config()
    model = str(cfg.get("model") or "").strip()
    provider = str(cfg.get("provider") or "").strip()
    base_url = str(cfg.get("base_url") or "").strip()
    api_key = str(cfg.get("api_key") or "").strip()
    configured = bool(model and (provider or base_url or api_key))
    if not configured:
        return {"configured": False, "model": model, "provider": provider, "label": ""}
    ...
    try:
        ...
        runtime_provider = str(runtime.get("provider") or provider or "")
        api_mode = str(runtime.get("api_mode") or "")
    except Exception:
        # Info endpoint should still render the configured target even if runtime
        # credential resolution cannot complete until the first chat request.
        logger.debug("delegation info runtime resolution failed", exc_info=True)
    return {
        "configured": True,
        "model": model,
...
def _handle_delegation_info(handler):
    return j(handler, _delegation_info_payload())


def _normalized_delegation_messages(messages: object) -> list[dict]:
...
def _json_request(url: str, payload: dict, headers: dict, timeout: int = 120) -> dict:
    import urllib.error
    ...
    try:
        ...
        try:
            ...
            err_data = {"error": raw[:500]}
        detail = err_data.get("error") if isinstance(err_data, dict) else err_data
        if isinstance(detail, dict):
            detail = detail.get("message") or detail.get("type") or json.dumps(detail)
        raise RuntimeError(f"provider returned HTTP {e.code}: {detail or 'unknown error'}") from e
    return json.loads(raw) if raw else {}


def _chat_completion_reply(runtime: dict, model: str, messages: list[dict]) -> str:
    ...
    if not base_url:
        raise RuntimeError("delegation runtime has no base_url")
    headers = {"Content-Type": "application/json"}
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"
    if str(runtime.get("provider") or "").lower() == "openrouter":
        headers.setdefault("HTTP-Referer", "http://localhost")
        headers.setdefault("X-Title", "Hermes WebUI Delegation")
    data = _json_request(
...
def _anthropic_messages_reply(runtime: dict, model: str, messages: list[dict]) -> str:
    base_url = str(runtime.get("base_url") or "https://api.anthropic.com").rstrip("/")
    api_key = str(runtime.get("api_key") or "")
    if not api_key:
        raise RuntimeError("delegation runtime has no Anthropic API key")
...
def _handle_delegation_chat(handler, body):
    try:
        messages = _normalized_delegation_messages(body.get("messages"))
    except ValueError as e:
        return bad(handler, str(e))

    info = _delegation_info_payload()
    if not info.get("configured"):
        return bad(handler, "delegation model not configured", 400)
    cfg = _delegation_config()
    model = str(cfg.get("model") or "").strip()
    provider = str(cfg.get("provider") or "").strip()
    base_url = str(cfg.get("base_url") or "").strip()
    ...
    try:
        ...
        if api_mode == "anthropic_messages":
            reply = _anthropic_messages_reply(runtime, model, messages)
        elif api_mode in {"chat_completions", "openai_chat_completions", "responses"}:
            reply = _chat_completion_reply(runtime, model, messages)
        else:
            return bad(handler, f"delegation api_mode '{api_mode}' is not supported by direct chat", 400)
        return j(handler, {"ok": True, "success": True, "reply": reply, "model": model, "provider": runtime.get("provider")})
    except Exception as e:
        logger.warning("delegation chat failed: %s", _sanitize_error(e))
...
def _parse_ideas_sections(content):
    """Parse project-ideas.md content into sections array."""
    lines = content.split("\n")
    sections = []
    ...
    for line in lines:
        if line.startswith("## "):
            current_section = {"name": line.replace("## ", "").strip(), "entries": []}
            sections.append(current_section)
            current_entry = None
        elif line.startswith("### ") and current_section is not None:
            entry_name = line.replace("### ", "").strip()
            current_entry = {"name": entry_name, "lines": [line]}
...
<a href="/">← Back to Pantheon</a>
    ...
    if parsed.path == "/api/system/health":
        from api.god_runtime import _get_god_state, refresh_god_statuses
        from api.profiles import list_profiles_api

        payload = build_system_health_payload()

        # Scan for hermes agent process
        try:
            import subprocess
            ...
            for line in ps_out.splitlines():
                if 'gateway' in line and 'run' in line and 'hermes' in line:
                    hermes_running = True
                if ('mcp_server' in line or 'mcp-server' in line) and 'python' in line:
                    mcp_running = True
                if 'litellm' in line.lower():
                    litellm_running = True

            # Uptime from /proc/1 or os
            ...
            payload['hermes'] = {
                ...
                'uptime': f'{int(boot_time // 86400)}d {int((boot_time % 86400) // 3600)}h ago' if boot_time and hermes_running else None,
            }
            payload['mcp'] = {
                'status': 'connected' if mcp_running else 'disconnected',
            }
            payload['litellm'] = {
                'status': 'running' if litellm_running else 'stopped',
            }

            # God runtimes — 5-state model with heartbeat detection
            ...
            gods = []
            heartbeat_dir = _Path.home() / ".hermes" / "gods"
            for p in profiles:
                god_md = p.get("god", {})
                name = p["name"]
                state = _get_god_state(name)
                proc_alive = state.get("state") == "awake"
                pid = state.get("pid")

                # Read heartbeat file
                hb = None
                hb_file = heartbeat_dir / name / "heartbeat.json"
                try:
                    if hb_file.exists():
                ...
                now = _time_mod.time()
                hb_age = (now - hb["timestamp"]) if hb and hb.get("timestamp") else None

                # Determine 5-state status
                if proc_alive:
                    if hb and hb_age is not None and hb_age < 300:
                        if hb_age < 30:
                            god_status = "active"      # Running + heartbeat <30s
                        else:
                            god_status = "sleeping"    # Running + heartbeat 30s-5min
                    else:
                        god_status = "idle"            # Running but no recent heartbeat
                elif hb is not None:
                    god_status = "dead"                 # Process dead but heartbeat exists (was alive)
                else:
                    god_status = "ready"                # Never started / no process + no heartbeat

                # Calculate last active time for display
                last_active = None
                if hb and hb.get("iso_timestamp"):
                    last_active = hb.get("iso_timestamp")
                    # Show relative time
                    if hb_age is not None:
                        if hb_age < 60:
                            last_active = "just now"
                        elif hb_age < 3600:
                            last_active = f"{int(hb_age // 60)}m ago"
                        elif hb_age < 86400:
                            last_active = f"{int(hb_age // 3600)}h ago"
                        else:
                            last_active = f"{int(hb_age // 86400)}d ago"
    ...
    if parsed.path == "/api/stream/entities":
        return j(handler, get_stream_entities())

    if parsed.path == "/api/stream/edges":
        return j(handler, get_stream_edges())
    ...
    if parsed.path == "/api/work-items":
        return _handle_work_items_get(handler)

    if parsed.path == "/api/delegation/info":
        return _handle_delegation_info(handler)

    if parsed.path == "/api/tools/toolsets":
        return _handle_toolsets_list(handler)
    ...
    if parsed.path == "/api/skills/trash":
        return _handle_skill_trash(handler)
    ...
    if parsed.path == "/api/gods/summon/list":
        ...
        repo = "Duskript/Pantheon-Summons"
        url = f"https://api.github.com/repos/{repo}/contents/gods"
        req = _urllib.Request(url, headers={"User-Agent": "Pantheon/1.0"})
        try:
        ...
        for g in god_dirs:
            name = g["name"]
            soul_url = f"https://raw.githubusercontent.com/{repo}/main/gods/{name}/SOUL.md"
            try:
                soul_req = _urllib.Request(soul_url, headers={"User-Agent": "Pantheon/1.0"})
                with _urllib.urlopen(soul_req, timeout=10) as sresp:
        ...
        try:
            repo_req = _urllib.Request(f"https://api.github.com/repos/{repo}", headers={"User-Agent": "Pantheon/1.0"})
            with _urllib.urlopen(repo_req, timeout=10) as rresp:
                repo_info = _json.loads(rresp.read().decode("utf-8"))
        except Exception:
            repo_info = {}
    ...
    # ── God Roster (GET) ──
    # Returns all registered gods with their metadata and real gateway status
    ...
    if parsed.path.startswith("/api/gods/") and '/' not in parsed.path[len("/api/gods/"):]:
        ...
        god_name = parsed.path[len("/api/gods/"):]
        profiles = list_profiles_api()
        for p in profiles:
            if p["name"] == god_name:
                god_md = p.get("god", {})
                state = _get_god_state(god_name)
                return j(handler, {
                    "name": p["name"],
                    "display_name": god_md.get("display_name", p["name"].capitalize()),
                    "icon": god_md.get("icon", "🧑‍💻"),
                    "color": god_md.get("color", "#748FFC"),
                    "domain": god_md.get("domain", "Specialist"),
                    "is_active": p.get("is_active", False),
        ...
        return j(handler, {"error": f"God '{god_name}' not found"}, status=404)

    # ── God SOUL.md (GET) ──
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/soul"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[:-len("/soul")]
        ...
        if not soul_path.is_file():
            return j(handler, {"exists": False, "content": None})
        try:
            content = soul_path.read_text(encoding="utf-8")
            return j(handler, {"exists": True, "content": content})
        except OSError as e:
            return bad(handler, f"Failed to read SOUL.md: {e}")

    # ── God Icon (GET) ──
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/icon"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[:-len("/icon")]
        from api.profiles import _resolve_profile_home_for_name
        ...
        try:
            ...
            return bad(handler, f"Could not resolve profile for '{god_name}'")
        icon_path = god_home / "icon.png"
        if not icon_path.is_file():
            return j(handler, {"error": "No icon set"}, status=404)
        try:
            icon_data = icon_path.read_bytes()
            # Detect image format from magic bytes for correct Content-Type
            ctype = "image/png"
            if icon_data[:4] == b"\x89PNG":
                ...
                ctype = "image/webp"
            elif icon_data[:3] == b"GIF":
                ctype = "image/gif"
            handler.send_response(200)
            handler.send_header("Content-Type", ctype)
            handler.send_header("Content-Length", str(len(icon_data)))
            handler.send_header("Cache-Control", "no-cache")
            handler.end_headers()
            handler.wfile.write(icon_data)
            return True
        except OSError as e:
            return bad(handler, f"Failed to read icon: {e}")

    # ── God Codex Folders (GET) ──
    ...
    if parsed.path == "/api/athenaeum/list":
        from api.athenaeum import list_codexes

        query = parse_qs(parsed.query)
        details = query.get("details", [""])[0].lower() in {"1", "true", "yes", "on"}
        return j(handler, list_codexes(details=details))

    if parsed.path == "/api/athenaeum/walk":
        from api.athenaeum import walk

        query = parse_qs(parsed.query)
        path = query.get("path", ["INDEX.md"])[0]
        return j(handler, walk(path=path))

    if parsed.path == "/api/athenaeum/read":
        from api.athenaeum import read

        query = parse_qs(parsed.query)
        path = query.get("path", [""])[0]
        if not path:
            return bad(handler, "path parameter is required")
        return j(handler, read(path=path))

    if parsed.path == "/api/athenaeum/search":
        from api.athenaeum import search

        query = parse_qs(parsed.query)
        q = query.get("q", [""])[0]
        if not q:
            return bad(handler, "q parameter is required")
        n_results = 5
        try:
            n_results = min(max(int(query.get("n", ["5"])[0]), 1), 20)
        except (ValueError, TypeError):
            n_results = 5
        codexes_raw = query.get("codex", [""])[0]
        codexes = [c.strip() for c in codexes_raw.split(",") if c.strip()] if codexes_raw else None
        return j(handler, search(query=q, codexes=codexes, n_results=n_results))

    if parsed.path == "/api/athenaeum/graph-search":
        from api.athenaeum import graph_search
    ...
    if parsed.path == "/api/profile/active":
        ...
        return j(
            handler,
            {
                ...
                "path": str(active_home),
                "god": _read_god_metadata(active_home),
            },
        )

    # ── Boons API ─────────────────────────────────────────────────────────────
    if parsed.path == "/api/boons/list":
        from api.boons import list_boons

        query = parse_qs(parsed.query)
        god = query.get("god", [""])[0]
        pinned_only = query.get("pinned", [""])[0].lower() in {"1", "true", "yes"}
        limit = 50
        try:
        ...
        try:
            ...
            offset = 0
        return j(handler, list_boons(god=god, pinned_only=pinned_only, limit=limit, offset=offset))

    if parsed.path == "/api/boons/get":
        from api.boons import get_boon

        query = parse_qs(parsed.query)
        boon_id = query.get("id", [""])[0]
        if not boon_id:
            return bad(handler, "id parameter is required")
        boon = get_boon(boon_id)
        if not boon:
            return bad(handler, f"Boon not found: {boon_id}", status=404)
        return j(handler, boon)

    # ── Pantheon Naming Adapter (GET) ──
    # Returns the mapping from internal Hermes terms to Pantheon display terms.
    # Frontend can consume this to dynamically label UI elements. The internal
    # API field names remain unchanged — only UI labels are remapped.
    if parsed.path == "/api/pantheon/mapping":
        return j(handler, {
            "mapping": {
                "session": "Communion",
                "chat": "Communion",
            ...
            "agent_name": "Hermes",
            "note": "Internal API field names stay unchanged. Frontend i18n handles display renaming.",
        })
    ...
    if parsed.path == "/api/ideas":
        ...
        if not os.path.isfile(ideas_path):
            return j(handler, {"ideas": "", "error": None})
        try:
            with open(ideas_path, "r", encoding="utf-8") as f:
                content = f.read()
            sections = _parse_ideas_sections(content)
            return j(handler, {"ideas": content, "sections": sections, "error": None})
        except OSError as e:
            return j(handler, {"ideas": "", "sections": [], "error": str(e)})

    # ── GET push subscription status (is there a subscription to send to?) ──
    if parsed.path == "/api/push/status":
        push_config_path = os.path.expanduser("~/.hermes/push-config.json")
        try:
            push_config = json.loads(Path(push_config_path).read_text())
            subs = push_config.get("subscriptions", [])
            vapid_public = push_config.get("vapid", {}).get("public", "")
            return j(handler, {"subscribed": len(subs) > 0, "count": len(subs), "vapid_public": vapid_public})
        except Exception:
            return j(handler, {"subscribed": False, "count": 0, "vapid_public": ""})

    # ── Pantheon Notifications Poll (GET) ──
    if parsed.path == "/api/notifications/poll":
        import json as _json
        notif_file = os.path.expanduser("~/.hermes/notifications.json")
        if not os.path.isfile(notif_file):
            return j(handler, {"notifications": []})
        try:
            existing = _json.loads(Path(notif_file).read_text())
...
# ── Session Update (POST) ─────────────────────────────────────────────────
    ...
    # ── Chat stream control (POST compatibility) ──
    if parsed.path == "/api/chat/cancel":
        return _handle_chat_cancel_post(handler, body)
    ...
    if parsed.path == "/api/workspaces/switch":
        return _handle_workspace_switch(handler, body)
    ...
    if parsed.path == "/api/work-items":
        return _handle_work_items_update(handler, body)

    if parsed.path == "/api/delegation/chat":
        return _handle_delegation_chat(handler, body)
    ...
    if parsed.path == "/api/skills/restore":
        return _handle_skill_restore(handler, body)

    # ── Memory (POST) ──
    if parsed.path == "/api/memory/write":
        return _handle_memory_write(handler, body)

    # ── God Management: SOUL.md Write (POST) ──
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/soul"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[:-len("/soul")]
        content = body.get("content")
        if content is None:
            return bad(handler, "content field is required")
        from api.profiles import _resolve_profile_home_for_name
        ...
        try:
            ...
            return bad(handler, f"Failed to write SOUL.md: {e}")

    # ── God Management: Metadata Update (POST) ──
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/metadata"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[:-len("/metadata")]
        from api.profiles import _resolve_profile_home_for_name, _read_god_metadata, _write_god_metadata

        try:
            god_home = _resolve_profile_home_for_name(god_name)
        except Exception:
            return bad(handler, f"Could not resolve profile for '{god_name}'")
        existing = _read_god_metadata(god_home)
        for key in ("display_name", "icon", "color", "domain"):
            if key in body:
                existing[key] = str(body[key]).strip()
        try:
            _write_god_metadata(god_home, existing)
            return j(handler, {"ok": True, "god": existing})
        except OSError as e:
            return bad(handler, f"Failed to write metadata: {e}")

    # ── God Management: Model Update (POST) ──
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/model"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[:-len("/model")]
        model_id = body.get("model")
        if not model_id:
            return bad(handler, "model is required")
        try:
            result = set_god_model(god_name, str(model_id).strip())
            return j(handler, result)
        except ValueError as e:
            return bad(handler, str(e))
        except RuntimeError as e:
    ...
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/icon"):
        ...
        try:
            ...
            return bad(handler, f"Could not resolve profile for '{god_name}'")
        raw_data = body.get("image", "") or body.get("icon", "")
        if not raw_data:
            return bad(handler, "image field is required (base64-encoded PNG)")
        try:
            if "," in raw_data:
                raw_data = raw_data.split(",", 1)[1]
            icon_bytes = base64.b64decode(raw_data)
            icon_path = god_home / "icon.png"
            icon_path.write_bytes(icon_bytes)
            return j(handler, {"ok": True, "url": f"/api/gods/{god_name}/icon"})
        except (ValueError, OSError) as e:
            return bad(handler, f"Failed to save icon: {e}")

    # ── God Export (POST) — bundle for marketplace ──
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/export"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[:-len("/export")]
        if not god_name:
            return bad(handler, "god_name is required")

        pantheon_bundle = os.path.join(PANTHEON_DIR, "scripts", "pantheon-bundle")
        exports_path = Path(EXPORTS_DIR)
        exports_path.mkdir(parents=True, exist_ok=True)

        # Build command
        cmd = [sys.executable, pantheon_bundle, god_name, "--output", str(exports_path)]
        codex_folders = body.get("codex_folders")
        if codex_folders is not None:
            if isinstance(codex_folders, list) and len(codex_folders) > 0:
                cmd.append("--with-codex")
                cmd.extend(codex_folders)
            elif isinstance(codex_folders, list) and len(codex_folders) == 0:
                cmd.append("--with-codex")
        if body.get("include_skills") is False:
            cmd.append("--no-skills")

        tarball_data = None
        tarball_name = None

        # Try 1: pantheon-bundle for Pantheon-installed gods
        if os.path.isfile(pantheon_bundle):
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
                if result.returncode == 0:
                    tarballs = sorted(exports_path.glob(f"god-{god_name.replace('god-','')}-v*.tar.gz"))
                    if tarballs:
                        tb = tarballs[-1]
                        tarball_data = tb.read_bytes()
                        tarball_name = tb.name
            except Exception:
                pass

        # Try 2: Profile-based god fallback
        if tarball_data is None:
            from api.profiles import _resolve_profile_home_for_name
            try:
                god_home = _resolve_profile_home_for_name(god_name)
                if os.path.isdir(god_home):
                    ...
                    with _tarfile.open(fileobj=buf, mode="w:gz") as tar:
                        # SOUL.md
                        soul_path = os.path.join(god_home, "SOUL.md")
                        if os.path.isfile(soul_path):
                            tar.add(soul_path, arcname=f"god-{god_name}/SOUL.md")
                        # persona.md
                        persona_path = os.path.join(god_home, "persona.md")
                        if os.path.isfile(persona_path):
                            tar.add(persona_path, arcname=f"god-{god_name}/persona.md")
                        # god config (profiles.json or config.yaml)
                        config_path = os.path.join(god_home, "profiles.json")
                        if os.path.isfile(config_path):
                            tar.add(config_path, arcname=f"god-{god_name}/profiles.json")
                        config_yaml = os.path.join(god_home, "config.yaml")
                        if os.path.isfile(config_yaml):
                            tar.add(config_yaml, arcname=f"god-{god_name}/config.yaml")
                        # icon
                        icon_path = os.path.join(god_home, "icon.png")
                        ...
                        # Codex folders if requested
                        requested_folders = body.get("codex_folders")
                        if requested_folders and isinstance(requested_folders, list) and len(requested_folders) > 0:
                            athenaeum_root = os.path.expanduser("~/athenaeum")
                ...
                logger.error("Profile fallback export failed for %s: %s", god_name, e)

        if tarball_data is None:
            return bad(handler, f"Export failed: god '{god_name}' not found as Pantheon package or Hermes profile", status=404)

        handler.send_response(200)
        handler.send_header("Content-Type", "application/gzip")
        handler.send_header("Content-Disposition", f'attachment; filename="{tarball_name}"')
        handler.send_header("Content-Length", str(len(tarball_data)))
        handler.send_header("Cache-Control", "no-cache")
        handler.end_headers()
        handler.wfile.write(tarball_data)
    ...
    if parsed.path.startswith("/api/gods/") and parsed.path.endswith("/forge"):
        ...
        if len(parts) == 5 and parts[3]:
            ...
            from api.profiles import _resolve_profile_home_for_name, _read_god_metadata
            ...
            if action == "start":
                ...
                if not msg:
                    return bad(handler, "message field is required")
                result = forge_chat(god_name, god_domain, msg)
                return j(handler, result)
                ...
                return bad(handler, f"Unknown forge action: {action}")

        return bad(handler, "Invalid forge path", 400)

    # ── God Summon (POST) — Install a god from Pantheon-Summons repo ──
    if parsed.path == "/api/gods/summon":
        import urllib.request

        god_name = body.get("god_name", "").strip()
        repo = body.get("repo", "Duskript/Pantheon-Summons")
        if not god_name:
            return bad(handler, "god_name is required")

        # Sanitize god name
        import re as _re
        safe_name = god_name.lower().replace(" ", "-")
        if not _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", safe_name):
            return bad(handler, f"Invalid god name: {god_name}")

        try:
            # Fetch SOUL.md from GitHub
            url = f"https://raw.githubusercontent.com/{repo}/main/gods/{god_name}/SOUL.md"
            req = urllib.request.Request(url, headers={"User-Agent": "Pantheon/1.0"})
            with urllib.request.urlopen(req, timeout=15) as resp:
                soul_md = resp.read().decode("utf-8")
        except urllib.error.HTTPError as e:
            if e.code == 404:
                return bad(handler, f"God '{god_name}' not found in Pantheon-Summons repo")
            return bad(handler, f"Failed to fetch god manifest: HTTP {e.code}")
        except Exception as e:
            return bad(handler, f"Network error: {str(e)}")
        ...
        try:
            ...
            return j(handler, {"error": f"God '{safe_name}' already exists"})
        except (ValueError, RuntimeError) as e:
            return bad(handler, str(e))

        # Write the SOUL.md to the god's directory
        try:
            from api.profiles import _resolve_profile_home_for_name
            god_home = _resolve_profile_home_for_name(safe_name)
            soul_path = os.path.join(god_home, "SOUL.md")
    ...
    if parsed.path.endswith("/submit-to-summons"):
        suffix = parsed.path[len("/api/gods/"):]
        god_name = suffix[: -len("/submit-to-summons")]
        if not god_name:
            return bad(handler, "god_name is required")
        ...
        try:
            ...
            return bad(handler, f"God '{god_name}' not found")

        # Read SOUL.md
        soul_path = os.path.join(god_home, "SOUL.md")
        if not os.path.isfile(soul_path):
            return bad(handler, f"No SOUL.md found for '{god_name}' — forge one first")
        ...
        # Parse optional body params for what to include in the review tarball
        codex_folders = body.get("codex_folders") if isinstance(body, dict) else None
        include_skills = body.get("include_skills") if isinstance(body, dict) else True
        if include_skills is None:
            include_skills = True

        # Build a review tarball with the full god bundle (same logic as export)
        import tarfile as _tarfile
        import io as _io
        import base64

        tarball_buf = _io.BytesIO()
        tarball_uploaded = False
        with _tarfile.open(fileobj=tarball_buf, mode="w:gz") as tar:
            # SOUL.md
            if os.path.isfile(soul_path):
                tar.add(soul_path, arcname=f"god-{god_name}/SOUL.md")
            # persona.md
            _persona_path = os.path.join(god_home, "persona.md")
            if os.path.isfile(_persona_path):
                tar.add(_persona_path, arcname=f"god-{god_name}/persona.md")
            # config
            _config_path = os.path.join(god_home, "profiles.json")
            if os.path.isfile(_config_path):
                tar.add(_config_path, arcname=f"god-{god_name}/profiles.json")
            _config_yaml = os.path.join(god_home, "config.yaml")
            if os.path.isfile(_config_yaml):
                tar.add(_config_yaml, arcname=f"god-{god_name}/config.yaml")
            # icon
            _icon_check = os.path.join(god_home, "icon.png")
            if os.path.isfile(_icon_check):
                tar.add(_icon_check, arcname=f"god-{god_name}/icon.png")
            # skills if requested
            if include_skills:
                _skills_dir = os.path.join(god_home, "skills")
                if os.path.isdir(_skills_dir):
                    tar.add(_skills_dir, arcname=f"god-{god_name}/skills")
            # codex folders if requested
            _athenaeum_root = os.path.expanduser("~/athenaeum")
            if codex_folders and isinstance(codex_folders, list) and len(codex_folders) > 0:
                for _folder in codex_folders:
                    _folder_path = os.path.join(_athenaeum_root, _folder)
                    if os.path.isdir(_folder_path):
                        tar.add(_folder_path, arcname=f"god-{god_name}/codex/{_folder}")
        ...
        commit_msg = f"summon({god_name}): add SOUL.md and review bundle"
        pr_title = f"🌟 Summon {god_name}"
        pr_body = (
            f"This PR summons **{god_name}** to the Pantheon.\n\n"
            f"Automatically submitted via the Pantheon Export flow.\n\n"
        )
        # Build included-content summary for PR body
        _content_parts = ["SOUL.md"]
        if os.path.isfile(os.path.join(god_home, "icon.png")):
            _content_parts.append("icon.png")
        if os.path.isfile(os.path.join(god_home, "persona.md")):
            _content_parts.append("persona.md")
        if os.path.isfile(os.path.join(god_home, "profiles.json")) or os.path.isfile(os.path.join(god_home, "config.yaml")):
            _content_parts.append("config")
        if include_skills and os.path.isdir(os.path.join(god_home, "skills")):
            _content_parts.append("skills")
        if codex_folders and isinstance(codex_folders, list) and len(codex_folders) > 0:
            _content_parts.append(f"codex ({len(codex_folders)} folder{'s' if len(codex_folders) > 1 else ''})")
        pr_body += f"**Bundle includes:** {', '.join(_content_parts)}.\n\n"
        pr_body += (
            f"A full review tarball (`god-{god_name}-review.tar.gz`) is attached to this branch "
            f"so reviewers can inspect the complete god bundle including knowledge bases.\n"
        )

        try:
            # 1. Create branch from main
            r = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/refs/heads/main"],
                capture_output=True, text=True, timeout=15
            )
            if r.returncode != 0:
                return bad(handler, f"Failed to get main ref: {r.stderr.strip()}")
            main_sha = json.loads(r.stdout)["object"]["sha"]

            # Check if branch already exists
            r2 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/refs/heads/{branch}"],
                capture_output=True, text=True, timeout=10
            )
            if r2.returncode == 0:
                return bad(handler, f"Branch '{branch}' already exists — a PR may already be open for this god")

            # Create the branch
            r3 = subprocess.run(
                ["gh", "api", "repos/{}/git/refs".format(SUMMONS_REPO), "-f", "ref=refs/heads/{}".format(branch), "-f", "sha={}".format(main_sha), "--method", "POST"],
                capture_output=True, text=True, timeout=15
            )
            if r3.returncode != 0 and "already exists" not in r3.stderr:
                # Might be a race condition where branch was created between checks
                r3 = subprocess.run(
                    ["gh", "api", f"repos/{SUMMONS_REPO}/git/refs/heads/{branch}"],
                    capture_output=True, text=True, timeout=10
                )
                if r3.returncode != 0:
                    return bad(handler, f"Failed to create branch: {r3.stderr.strip()}")

            # 2. Create or update the SOUL.md file on the new branch
            # First get the base tree from main
            r4 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/trees/{main_sha}"],
                capture_output=True, text=True, timeout=15
            )
            if r4.returncode != 0:
                return bad(handler, f"Failed to get tree: {r4.stderr.strip()}")

            # This submit PR also ONLY handled SOUL.md — no icon. Fix that.
            # ── Check for and upload icon.png ──
            icon_path = os.path.join(god_home, "icon.png")
            icon_b64 = None
            if os.path.isfile(icon_path):
                with open(icon_path, "rb") as ictx:
                    icon_b64 = base64.b64encode(ictx.read()).decode("ascii")

            # Create blob with SOUL.md content
            encoded_soul = soul_md.encode("utf-8")
            soul_b64 = base64.b64encode(encoded_soul).decode("ascii")
            soul_payload = json.dumps({"content": soul_b64, "encoding": "base64"})
            r5 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/blobs", "--method", "POST", "--input", "-"],
                input=soul_payload, capture_output=True, text=True, timeout=15
            )
            if r5.returncode != 0:
                return bad(handler, f"Failed to create blob: {r5.stderr.strip()}")
            soul_blob_sha = json.loads(r5.stdout)["sha"]

            # Upload icon blob if present
            icon_blob_sha = None
            if icon_b64:
                icon_payload = json.dumps({"content": icon_b64, "encoding": "base64"})
                r_icon = subprocess.run(
                    ["gh", "api", f"repos/{SUMMONS_REPO}/git/blobs", "--method", "POST", "--input", "-"],
                    input=icon_payload, capture_output=True, text=True, timeout=15
                )
                if r_icon.returncode == 0:
                    icon_blob_sha = json.loads(r_icon.stdout)["sha"]

            # ── Upload review tarball blob ──
            tarball_blob_sha = None
            if tarball_b64:
                tarball_payload = json.dumps({"content": tarball_b64, "encoding": "base64"})
                r_tarball = subprocess.run(
                    ["gh", "api", f"repos/{SUMMONS_REPO}/git/blobs", "--method", "POST", "--input", "-"],
                    input=tarball_payload, capture_output=True, text=True, timeout=120
                )
                if r_tarball.returncode == 0:
                    try:
                        tarball_blob_sha = json.loads(r_tarball.stdout)["sha"]
                    except (json.JSONDecodeError, KeyError):
                        tarball_blob_sha = None

            # Create tree with the new file(s)
            tree_entries = [
                {
                    "path": f"gods/{god_name}/SOUL.md",
                    "mode": "100644",
                    "type": "blob",
                    "sha": soul_blob_sha
                }
            ...
            if tarball_blob_sha:
                tree_entries.append({
                    "path": f"gods/{god_name}/god-{god_name}-review.tar.gz",
                    "mode": "100644",
                    "type": "blob",
                    "sha": tarball_blob_sha
                })
            ...
            r6 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/trees", "--method", "POST", "--input", "-"],
                input=tree_payload, capture_output=True, text=True, timeout=15
            )
            if r6.returncode != 0:
                return bad(handler, f"Failed to create tree: {r6.stderr.strip()}")
            new_tree_sha = json.loads(r6.stdout)["sha"]
            ...
            r7 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/commits", "--method", "POST", "--input", "-"],
                input=commit_payload, capture_output=True, text=True, timeout=15
            )
            if r7.returncode != 0:
                return bad(handler, f"Failed to create commit: {r7.stderr.strip()}")
            commit_sha = json.loads(r7.stdout)["sha"]

            # Update branch ref to point to new commit
            r8 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/git/refs/heads/{branch}", "-f", f"sha={commit_sha}", "--method", "PATCH"],
                capture_output=True, text=True, timeout=15
            )
            if r8.returncode != 0 and "already" not in r8.stderr:
                return bad(handler, f"Failed to update ref: {r8.stderr.strip()}")
            ...
            r9 = subprocess.run(
                ["gh", "api", f"repos/{SUMMONS_REPO}/pulls", "--method", "POST", "--input", "-"],
                input=pr_payload, capture_output=True, text=True, timeout=15
            )
            if r9.returncode != 0:
                return bad(handler, f"Failed to create PR: {r9.stderr.strip()}")

            pr_data = json.loads(r9.stdout)
            pr_url = pr_data.get("html_url", f"https://github.com/{SUMMONS_REPO}/pull/{pr_data.get('number', '')}")

            return j(handler, {
                "ok": True,
                "pr_url": pr_url,
                "message": f"🌟 {god_name} submitted! PR created at {pr_url}",
            })

        except subprocess.TimeoutExpired:
            return bad(handler, "GitHub API timed out", status=504)
        except Exception as e:
            return bad(handler, f"Submission failed: {str(e)}")
        ...
        if len(parts) == 5 and parts[3] and parts[4] in ("invoke", "sleep", "exile"):
            ...
            if action == "invoke":
                ...
                try:
                    ...
                    if god_name != 'default':
                        _validate_profile_name(god_name)
                    switch_profile(god_name, process_wide=False)
                    from api.config import invalidate_models_cache
                    invalidate_models_cache()
                    extra_headers = {'Set-Cookie': build_profile_cookie(god_name)}
                except Exception:
                    extra_headers = None
    ...
    if parsed.path == "/api/gods/refresh":
        refresh_god_statuses()
        return j(handler, {"ok": True, "message": "Status refreshed"})
    ...
    if parsed.path == "/api/ideas/status":
        try:
            ...
            ideas_path = os.path.expanduser("~/pantheon/project-ideas.md")
            if not os.path.isfile(ideas_path):
                return j(handler, {"error": "Ideas file not found"})
            with open(ideas_path, "r", encoding="utf-8") as f:
                lines = f.readlines()
            new_lines = []
            in_section = False
            ...
            if not updated:
                return j(handler, {"error": "Entry not found", "sections": []})
            with open(ideas_path, "w", encoding="utf-8") as f:
                f.writelines(new_lines)
            with open(ideas_path, "r", encoding="utf-8") as f:
                content = f.read()
            sections = _parse_ideas_sections(content)
            return j(handler, {"success": True, "sections": sections})
        except Exception as e:
            logger.exception("ideas status update failed")
            return j(handler, {"error": str(e), "sections": []})

    # ── Ideas Delete Entry (POST) ──
    if parsed.path == "/api/ideas/delete":
        try:
            section_name = (body.get("section_name") or "").strip()
            entry_name = (body.get("entry_name") or "").strip()
            ...
            for line in lines:
                if line.startswith("## "):
                    ...
                    skipping = line[4:].strip() == entry_name
                if not skipping:
                    new_lines.append(line)
            ...
            logger.exception("ideas delete failed")
            return j(handler, {"error": str(e), "sections": []})

    # ── Ideas Reorder Entry (POST) ──
    if parsed.path == "/api/ideas/reorder":
        try:
            section_name = (body.get("section_name") or "").strip()
            if not section_name:
                return j(handler, {"error": "Missing/invalid fields", "sections": []})
            ideas_path = os.path.expanduser("~/pantheon/project-ideas.md")
            if not os.path.isfile(ideas_path):
                return j(handler, {"error": "Ideas file not found"})
            with open(ideas_path, "r", encoding="utf-8") as f:
            ...
            for i, line in enumerate(lines):
                if line.startswith("## ") and line[3:].strip() == section_name:
                    section_start = i
                elif line.startswith("## ") and section_start >= 0 and i > section_start:
                    section_end = i
                    break
            if section_start == -1:
                return j(handler, {"error": "Section not found"})
            # Extract entry blocks within section content
            ...
            # Support both old direction-based and new order-list-based formats
            order_list = body.get("order")
            if order_list and isinstance(order_list, list) and len(order_list) > 0:
                # New format: full order array
                # Build a map of entry_name -> block
                entry_map = {}
                for block in entry_blocks:
                    name = block[0][4:].strip() if block else ""
                    if name:
                        entry_map[name] = block
                # Reorder according to the provided list
                new_blocks = []
                for name in order_list:
                    if name in entry_map:
                        new_blocks.append(entry_map[name])
                # Preserve any entries not in the order list (append at end)
                for block in entry_blocks:
                    name = block[0][4:].strip() if block else ""
                    if name not in order_list:
                        new_blocks.append(block)
                entry_blocks = new_blocks
            else:
                # Old format: direction-based (up/down)
                entry_name = (body.get("entry_name") or "").strip()
                direction = (body.get("direction") or "").strip()
                if not entry_name or direction not in ("up", "down"):
                    return j(handler, {"error": "Missing/invalid fields", "sections": []})
                # Find target entry index
                target_idx = -1
                for i, block in enumerate(entry_blocks):
                    if block and block[0][4:].strip() == entry_name:
                        target_idx = i
                        break
                if target_idx == -1:
                    return j(handler, {"error": "Entry not found in section"})
                if direction == "up" and target_idx == 0:
                    return j(handler, {"error": "Already at top"})
                if direction == "down" and target_idx == len(entry_blocks) - 1:
                    return j(handler, {"error": "Already at bottom"})
                swap_idx = target_idx - 1 if direction == "up" else target_idx + 1
                entry_blocks[target_idx], entry_blocks[swap_idx] = entry_blocks[swap_idx], entry_blocks[target_idx]

            # Rebuild section body
            new_section_body = list(leading)
            for block in entry_blocks:
                new_section_body.extend(block)
            new_lines = lines[:section_start + 1] + new_section_body + lines[section_end:]
            with open(ideas_path, "w", encoding="utf-8") as f:
                f.writelines(new_lines)
            with open(ideas_path, "r", encoding="utf-8") as f:
                content = f.read()
            ...
            logger.exception("ideas reorder failed")
            return j(handler, {"error": str(e), "sections": []})

    # ── Ideas Add Entry (POST) ──
    if parsed.path == "/api/ideas/add":
        try:
            section_name = (body.get("section_name") or "").strip()
            entry_name = (body.get("entry_name") or "").strip()
            ...
            if idx == -1:
                return j(handler, {"error": f"Section '{section_name}' not found", "sections": []})
            ...
            if section_end == -1:
                ...
                new_content = content[:section_end] + entry_block + content[section_end:]
            with open(ideas_path, "w", encoding="utf-8") as f:
                f.write(new_content)
            with open(ideas_path, "r", encoding="utf-8") as f:
                content = f.read()
            ...
            logger.exception("ideas add failed")
            return j(handler, {"error": str(e), "sections": []})

    # ── Ideas Edit Entry (POST) ──
    if parsed.path == "/api/ideas/edit":
        try:
            section_name = (body.get("section_name") or "").strip()
            entry_name = (body.get("entry_name") or "").strip()
            ...
            for line in lines:
                if line.startswith("## "):
                    in_section = line[3:].strip() == section_name
                    if not found_section and in_section:
                        found_section = True
                    skipping = False
                elif line.startswith("### ") and in_section and line[4:].strip() == entry_name:
                    skipping = True
                elif skipping:
                    if line.startswith("### ") or line.startswith("## ") or (line.strip() == "" and new_lines and new_lines[-1].strip() == ""):
                        skipping = False
                ...
                if not skipping:
                    new_lines.append(line)
            content_without_entry = "\n".join(new_lines)
            # Determine target section for the edit
            target_section = new_section if new_section else section_name
            ...
            idx = content_without_entry.find(section_header)
            if idx == -1:
                # Section doesn't exist — create it
                section_header_line = "\n## " + target_section + "\n\n"
                content_without_entry = content_without_entry.rstrip() + section_header_line
                idx = content_without_entry.find("## " + target_section)
            section_end = content_without_entry.find("\n## ", idx + len(section_header))
            if section_end == -1:
                new_content = content_without_entry.rstrip() + "\n" + entry_block
            else:
                new_content = content_without_entry[:section_end] + entry_block + content_without_entry[section_end:]
            with open(ideas_path, "w", encoding="utf-8") as f:
                f.write(new_content)
            with open(ideas_path, "r", encoding="utf-8") as f:
                content = f.read()
            ...
            logger.exception("ideas edit failed")
            return j(handler, {"error": str(e), "sections": []})

    # ── Pantheon Notifications (POST create / dismiss / clear) ──
    if parsed.path == "/api/notifications/dismiss":
        notif_file = os.path.expanduser("~/.hermes/notifications.json")
        nid = body.get("id", "")
        if not nid:
            return j(handler, {"error": "id is required"}, status=400)
        if not os.path.isfile(notif_file):
            return j(handler, {"error": "no notifications"}, status=404)
        try:
            existing = json.loads(Path(notif_file).read_text())
            for n in existing:
                if n.get("id") == nid:
                    n["dismissed"] = True
                    break
            Path(notif_file).write_text(json.dumps(existing))
            return j(handler, {"ok": True})
        except Exception as e:
    ...
    if parsed.path == "/api/notifications/clear":
        ...
        try:
            existing = json.loads(Path(notif_file).read_text())
            keep = [n for n in existing if not n.get("dismissed")]
            for n in keep:
                n["dismissed"] = True
            Path(notif_file).write_text(json.dumps(keep))
            return j(handler, {"ok": True})
        except Exception as e:
            return j(handler, {"error": str(e)}, status=500)

    # ── Pantheon Push Subscription (POST) ──
    if parsed.path == "/api/push/subscribe":
        try:
            raw = handler.rfile.read(int(handler.headers.get('Content-Length', 0)))
            sub_data = json.loads(raw) if raw else {}
        except Exception:
            sub_data = body or {}
        if not sub_data.get("endpoint"):
            return j(handler, {"error": "endpoint is required"}, status=400)
        push_config_path = os.path.expanduser("~/.hermes/push-config.json")
        try:
            push_config = json.loads(Path(push_config_path).read_text())
        except Exception:
            push_config = {"vapid": {}, "subscriptions": []}
        push_config.setdefault("subscriptions", [])
        # Replace subscription for same endpoint if exists
        push_config["subscriptions"] = [
            s for s in push_config["subscriptions"] if s.get("endpoint") != sub_data.get("endpoint")
        ]
        push_config["subscriptions"].append(sub_data)
        if len(push_config["subscriptions"]) > 10:
            push_config["subscriptions"] = push_config["subscriptions"][-10:]
        try:
            Path(push_config_path).parent.mkdir(parents=True, exist_ok=True)
            Path(push_config_path).write_text(json.dumps(push_config, indent=2))
            return j(handler, {"ok": True})
        except Exception as e:
            return j(handler, {"error": str(e)}, status=500)
    ...
    # ── Pantheon God Notification (POST) ──
    # Gods call this to push a notification when they complete a task or need to report.
    # Accepts: {god, title, body, type, icon} — god name is used to look up their icon.
    if parsed.path == "/api/notifications/god":
        data = body or {}
        god_name = data.get("god", "").strip()
        title = data.get("title", "")
        if not title:
            return j(handler, {"error": "title is required"}, status=400)
        # Look up god's icon from the god profiles
        ...
        if god_name and not god_icon:
            try:
                ...
                profiles = list_profiles_api()
                for p in profiles:
                    if p.get("name", "").strip().lower() == god_name.lower():
                        god_icon = (p.get("god") or {}).get("icon", "")
                        break
            except Exception:
                pass
        # Prepend god name to title
        display_name = god_name.capitalize() if god_name else "God"
        full_title = f"{god_icon} {display_name}: {title}" if god_icon else f"🎭 {display_name}: {title}"
        notif_type = data.get("type", "info")
        body_text = data.get("body", "")
        icon_map = {"error": "🔴", "warning": "🟡", "info": "🔔", "success": "🟢"}
        notif_icon = god_icon or icon_map.get(notif_type, "🔔")
        # Write to notifications.json
        notif_file = os.path.expanduser("~/.hermes/notifications.json")
        existing = []
        if os.path.isfile(notif_file):
            try:
                ...
                existing = []
        new_entry = {
            "id": f"n_{int(time.time())}_{uuid.uuid4().hex[:8]}",
            "type": notif_type,
            "icon": notif_icon,
            "title": full_title,
            "body": body_text,
            ...
            "dismissed": False,
            "source": f"god:{god_name.lower()}" if god_name else "god",
        }
        existing.insert(0, new_entry)
        if len(existing) > 50:
            existing[:] = existing[:50]
        try:
            Path(notif_file).parent.mkdir(parents=True, exist_ok=True)
            Path(notif_file).write_text(json.dumps(existing))
        except Exception as e:
            return j(handler, {"error": f"Failed to store: {e}"}, status=500)
        # Also fire push notification
        try:
            push_config_path = os.path.expanduser("~/.hermes/push-config.json")
            if Path(push_config_path).exists():
                push_raw = Path(push_config_path).read_text()
                push_config = json.loads(push_raw) if push_raw.strip() else {}
                subs = push_config.get("subscriptions", [])
                vapid_private = push_config.get("vapid", {}).get("private", "")
                vapid_public = push_config.get("vapid", {}).get("public", "")
                if subs and vapid_private:
                    push_payload = json.dumps({
                        "title": full_title,
                        "body": body_text,
                        "tag": f"pantheon-god-{int(time.time())}",
                        "data": {"url": "/", "type": notif_type, "god": god_name}
                    })
                    for sub in subs[:5]:
                        try:
                            ...
                            webpush(
                                ...
                                vapid_claims={"sub": "mailto:hermes@pantheon.local"}
                            )
                        except WebPushException as e:
                            if e.response and e.response.status_code in (410, 404):
                                push_config["subscriptions"] = [
                                    s for s in push_config["subscriptions"]
            ...
            pass
        return j(handler, {"ok": True, "notification": new_entry})
    ...
    # ── Boons API (POST) ──────────────────────────────────────────────────────
    if parsed.path == "/api/boons/create":
        from api.boons import create_boon

        title = body.get("title", "").strip()
        body_data = body.get("body", "").strip()
        source_god = body.get("source_god", "").strip()
        if not title or not body_data or not source_god:
            return bad(handler, "title, body, and source_god are required")
        boon_type = body.get("type", "text")
        boon_format = body.get("format", "plain")
        source_communion = body.get("source_communion", "")
        boon = create_boon(
            title=title,
            body=body_data,
        ...
        return j(handler, boon, status=201)

    if parsed.path == "/api/boons/update":
        from api.boons import update_boon

        boon_id = body.get("id", "").strip()
        if not boon_id:
            return bad(handler, "id is required")
        updates = {k: body[k] for k in ("title", "body", "pinned", "forge_task_id", "type", "format") if k in body}
        boon = update_boon(boon_id, updates)
        if not boon:
            return bad(handler, f"Boon not found: {boon_id}", status=404)
        return j(handler, boon)

    if parsed.path == "/api/boons/delete":
        from api.boons import delete_boon

        boon_id = body.get("id", "").strip()
        if not boon_id:
            return bad(handler, "id is required")
        if delete_boon(boon_id):
            return j(handler, {"ok": True})
        return bad(handler, f"Boon not found: {boon_id}", status=404)

    if parsed.path == "/api/boons/toggle-pin":
        from api.boons import toggle_pin

        boon_id = body.get("id", "").strip()
        if not boon_id:
            return bad(handler, "id is required")
        boon = toggle_pin(boon_id)
        if not boon:
            return bad(handler, f"Boon not found: {boon_id}", status=404)
        return j(handler, boon)

    if parsed.path == "/api/boons/promote-to-forge":
        from api.boons import promote_to_forge

        boon_id = body.get("id", "").strip()
        if not boon_id:
            return bad(handler, "id is required")
        forge_task_id = body.get("forge_task_id", "").strip()
        boon = promote_to_forge(boon_id, forge_task_id)
        if not boon:
            return bad(handler, f"Boon not found: {boon_id}", status=404)
        return j(handler, boon)

    # ── Connectors API (POST) ──
# REMOVED (ACI.dev):     if parsed.path == "/api/connectors/connect":
        # REMOVED (ACI.dev dead direction): from api.connectors import handle_post_connect
        return handle_post_connect(handler)

# REMOVED (ACI.dev):     if parsed.path == "/api/connectors/disconnect":
        # REMOVED (ACI.dev dead direction): from api.connectors import handle_post_disconnect
...
def _handle_list_dir(handler, parsed):
    ...
    # Session-scoped mode (existing contract): list relative to the session
    # workspace and return classic {entries, path} payload.
    if sid:
        try:
            s = get_session(sid)
    ...
    # Compatibility mode (no session_id): support workspace picker payload used
    # by the Hermes UI merge branch.
    try:
        raw_path = _strip_surrounding_quotes(str(qs.get("path", [""])[0]).strip())
        browse_path = raw_path or str(get_last_workspace())
        root = validate_workspace_to_add(browse_path)

        roots = []
        seen = set()
        for ws in load_workspaces():
            ws_path = str(ws.get("path", "")).strip()
            if not ws_path or ws_path in seen:
                continue
            seen.add(ws_path)
            roots.append(
                {
                    "name": str(ws.get("name") or Path(ws_path).name or "Workspace"),
                    "path": ws_path,
                }
            )
        ...
        parent_path = str(root.parent) if root.parent != root else ""
        return j(
            handler,
            {
                "path": str(root),
...
def _handle_chat_cancel_post(handler, body):
    """POST compatibility shim for chat cancel.

    Supports either:
      - {"stream_id": "..."} (preferred)
      - {"session_id": "..."} (legacy clients)
    """
    ...
    if not stream_id and session_id:
        try:
            ...
            return bad(handler, "Session not found", 404)
        stream_id = str(getattr(s, "active_stream_id", "") or "").strip()
        if not stream_id:
            return j(
                handler,
                {"ok": True, "cancelled": False, "session_id": session_id, "stream_id": None},
            )

    if not stream_id:
        return bad(handler, "stream_id or session_id required")

    cancelled = cancel_stream(stream_id)
    response = {"ok": True, "cancelled": cancelled, "stream_id": stream_id}
    if session_id:
        response["session_id"] = session_id
    return j(handler, response)


def _handle_workspace_switch(handler, body):
    """Set the active workspace path used for new chat turns.

    Compatibility endpoint for UIs that persist workspace selection separately
    from chat/start payloads.
    """
    ...
    set_last_workspace(str(resolved))
    return j(handler, {"ok": True, "path": str(resolved), "last": str(resolved)})
...
def _handle_workspace_add(handler, body):
    ...
    if any(w["path"] == str(p) for w in wss):
        return bad(handler, "Forge already in list");
...
def _skill_root_and_trash():
    from tools.skills_tool import SKILLS_DIR

    root = Path(SKILLS_DIR)
    trash = root.parent / _SKILL_TRASH_DIRNAME
    return root, trash


def _safe_local_skill_name(name: object) -> str:
...
def _find_local_skill_dir(skill_name: str, root: Path) -> Path | None:
    try:
    ...
    for skill_file in root.rglob("SKILL.md"):
        if skill_file.parent.name != skill_name:
            continue
        try:
            skill_file.parent.resolve().relative_to(root_resolved)
        except ValueError:
            continue
...
def _read_skill_trash_metadata(trash_dir: Path, trash_root: Path) -> dict:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    try:
        item_id = trash_dir.relative_to(trash_root).as_posix()
    except ValueError:
        item_id = trash_dir.name
    skill_file = trash_dir / "SKILL.md"
    name = str(metadata.get("original_name") or trash_dir.name.split("--", 1)[0])
    deleted_at = metadata.get("deleted_at")
    try:
        deleted_at_value = float(deleted_at) if deleted_at else 0
    except (TypeError, ValueError):
        deleted_at_value = 0
    item = {
        ...
        "name": name,
        "original_name": name,
        "folder_name": trash_dir.name,
        "original_path": str(metadata.get("original_relpath") or metadata.get("original_path") or name),
        "original_parent": str(metadata.get("original_parent") or ""),
        "deleted_at": _format_timestamp(deleted_at_value),
        "deleted_at_ts": deleted_at_value,
        "trash_path": str(trash_dir),
    }
    if skill_file.exists():
        try:
            first_lines = skill_file.read_text(encoding="utf-8").splitlines()[:40]
            description = ""
...
def _list_skill_trash_items() -> list[dict]:
    _root, trash_root = _skill_root_and_trash()
    if not trash_root.exists():
        return []
    items = []
    for skill_file in trash_root.rglob("SKILL.md"):
        items.append(_read_skill_trash_metadata(skill_file.parent, trash_root))
    items.sort(key=lambda row: float(row.get("deleted_at_ts") or 0), reverse=True)
    return items


def _handle_skill_trash(handler):
    items = _list_skill_trash_items()
    return j(handler, {"ok": True, "success": True, "items": items, "trash": items})
...
def _handle_skill_save(handler, body):
    ...
    return j(handler, {"ok": True, "success": True, "name": skill_name, "path": str(skill_file)})


def _handle_skill_delete(handler, body):
    try:
        require(body, "name")
        skill_name = _safe_local_skill_name(body.get("name"))
    except ValueError as e:
        return bad(handler, str(e))
    import shutil
    import uuid

    root, trash_root = _skill_root_and_trash()
    with _SKILL_TRASH_LOCK:
        skill_dir = _find_local_skill_dir(skill_name, root)
        if not skill_dir:
            return bad(handler, "Skill not found", 404)
        try:
        ...
        suffix = f"{skill_name}--{stamp}-{uuid.uuid4().hex[:8]}"
        dest_rel = rel.parent / suffix
        dest = trash_root / dest_rel
        dest.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(skill_dir), str(dest))
        deleted_at = time.time()
        meta = {
            "id": dest.relative_to(trash_root).as_posix(),
            "original_name": skill_name,
            "original_relpath": rel.as_posix(),
            "original_parent": rel.parent.as_posix() if rel.parent.as_posix() != "." else "",
            "deleted_at": deleted_at,
        }
        (dest / ".webui-trash.json").write_text(json.dumps(meta, indent=2, sort_keys=True), encoding="utf-8")
        item = _read_skill_trash_metadata(dest, trash_root)
    return j(handler, {"ok": True, "success": True, "name": skill_name, "trash_item": item, "path": str(dest / "SKILL.md")})


def _handle_skill_restore(handler, body):
    item_id = str(body.get("id") or body.get("trash_id") or "").strip()
    requested_name = str(body.get("name") or "").strip()
    if not item_id and not requested_name:
        return bad(handler, "id or name required")
    import shutil
    ...
    with _SKILL_TRASH_LOCK:
        candidates = []
        if item_id:
            candidate = (trash_root / item_id).resolve()
            try:
                candidate.relative_to(trash_root.resolve())
                if (candidate / "SKILL.md").exists():
                    candidates.append(candidate)
            except ValueError:
                return bad(handler, "Invalid trash id")
        if not candidates and requested_name:
            for skill_file in trash_root.rglob("SKILL.md"):
                item = _read_skill_trash_metadata(skill_file.parent, trash_root)
                if item.get("original_name") == requested_name or item.get("name") == requested_name:
                    candidates.append(skill_file.parent)
        if not candidates:
            return bad(handler, "Deleted skill not found", 404)
        trash_dir = candidates[0]
        item = _read_skill_trash_metadata(trash_dir, trash_root)
        rel = Path(str(item.get("original_path") or item.get("original_name") or trash_dir.name))
        if rel.is_absolute() or ".." in rel.parts:
            return bad(handler, "Invalid restore path")
        dest = root / rel
        try:
            ...
            return bad(handler, "Invalid restore path")
        if dest.exists():
            return bad(handler, f"Skill already exists at {rel.as_posix()}", 409)
        dest.parent.mkdir(parents=True, exist_ok=True)
        meta = trash_dir / ".webui-trash.json"
        if meta.exists():
            meta.unlink()
        shutil.move(str(trash_dir), str(dest))
        skill_file = dest / "SKILL.md"
    return j(handler, {"ok": True, "success": True, "name": dest.name, "path": str(skill_file)})
