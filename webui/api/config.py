...
def set_god_model(god_name: str, model_id: str) -> dict:
    """Persist the model for a specific god's profile config.yaml.

    Writes ``model.default`` and ``model.provider`` to the god's profile
    config file at ``~/.hermes/profiles/{god_name}/config.yaml``.  Does
    NOT reload the active profile's config — the god's own config is
    picked up on its next gateway restart or session start.
    """
    selected_model = str(model_id or "").strip()
    if not selected_model:
        raise ValueError("model is required")
    ...
    try:
        ...
        raise ValueError(f"Unknown god '{god_name}': {exc}") from exc

    config_path = god_home / "config.yaml"

    # Reuse the same lock since we're writing YAML.
    with _cfg_lock:
        config_data = _load_yaml_config_file(config_path)
        model_cfg = config_data.get("model", {})
        if not isinstance(model_cfg, dict):
    ...
    # Don't reload_config() here — that affects the active profile, not the god profile.
    invalidate_models_cache()
    return {"ok": True, "model": persisted_model, "provider": persisted_provider}
...
_SETTINGS_DEFAULTS = {
    ...
    "custom_theme": {},  # custom color overrides: { light: {...}, dark: {...} } or empty
}
