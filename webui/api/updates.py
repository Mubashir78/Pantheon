...
def _resolve_git_repo_root(path):
    """Resolve the actual git repository root for a path.

    Supports nested checkouts where REPO_ROOT points at a subdirectory inside
    a larger repo (e.g. /pantheon/webui inside /pantheon).
    """
    if path is None:
        return None
    ...
    # Direct checkout/worktree marker in this directory.
    if (path / '.git').exists():
        return path

    top, ok = _run_git(['rev-parse', '--show-toplevel'], path, timeout=5)
    if ok and top:
        try:
            resolved = Path(top).resolve()
            if (resolved / '.git').exists():
                return resolved
            # Worktrees can have a gitdir indirection; confirm with rev-parse.
            _, inside_ok = _run_git(['rev-parse', '--is-inside-work-tree'], resolved, timeout=5)
            if inside_ok:
                return resolved
        except Exception:
            return None
...
def _check_repo(path, name):
    ...
    repo_root = _resolve_git_repo_root(path)
    if repo_root is None:
        return None

    # Fetch latest from origin (network call, cached by TTL)
    _, fetch_ok = _run_git(['fetch', 'origin', '--quiet'], repo_root, timeout=15)
    if not fetch_ok:
    ...
    if ok and upstream:
        ...
        branch = _detect_default_branch(repo_root)
        compare_ref = f'origin/{branch}'

    # Count commits behind
    out, ok = _run_git(['rev-list', '--count', f'HEAD..{compare_ref}'], repo_root)
    ...
    mb_full, mb_ok = _run_git(['merge-base', 'HEAD', compare_ref], repo_root)
    if mb_ok and mb_full:
        short, ok = _run_git(['rev-parse', '--short', mb_full], repo_root)
        current = short if (ok and short) else None
    else:
        current = None
    latest, _ = _run_git(['rev-parse', '--short', compare_ref], repo_root)

    # Get repo URL for "What's new?" link
    remote_url, _ = _run_git(['remote', 'get-url', 'origin'], repo_root)
...
def apply_force_update(target: str) -> dict:
    ...
    try:
        ...
        _run_git(['checkout', '.'], repo_root)
        _, ok = _run_git(['reset', '--hard', compare_ref], repo_root)
        if not ok:
...
def _apply_update_inner(target):
    ...
    pull_out, pull_ok = _run_git(pull_args, repo_root, timeout=30)
    if not pull_ok:
        if stashed:
            _run_git(['stash', 'pop'], repo_root)
        ...
        if 'not possible to fast-forward' in pull_lower or 'diverged' in pull_lower:
            return {
                ...
                'message': (
                    ...
                    'git -C ' + str(repo_root) + ' reset --hard ' + compare_ref
                ),
                'diverged': True,
            }
        if 'does not track' in pull_lower or 'no tracking information' in pull_lower:
            return {
                ...
                'message': (
                    ...
                    'Run: git -C ' + str(repo_root) + ' branch --set-upstream-to=' + compare_ref
                ),
            }
    ...
    if stashed:
        _, pop_ok = _run_git(['stash', 'pop'], repo_root)
        if not pop_ok:
            return {
                'ok': False,
