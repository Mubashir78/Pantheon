...
_GOD_METADATA_FILE = 'god.json'

# ── Default gods / metadata ──────────────────────────────────────────────────
_DEFAULT_GODS = {
    'default': {
        'display_name': 'Hermes',
        'icon': '⚡',
        'color': '#F5C542',
        'domain': 'Messenger · Operations',
    },
    'hephaestus': {
    ...
    'apollo': {
        ...
        'icon': '☀️',
        'color': '#FFD43B',
        'domain': 'Knowledge · Light · Truth',
    },
    'hestia': {
        'display_name': 'Hestia',
        'icon': '🔥',
        'color': '#FF8787',
        'domain': 'Hearth · Cron · System',
    },
    'athena': {
        'display_name': 'Athena',
        'icon': '🦉',
    ...
    'dionysus': {
        ...
        'icon': '🍷',
        'color': '#DA77F2',
        'domain': 'Fun · Experiments',
    },
}


def _default_god_metadata(profile_name: str) -> dict:
    """Return default god metadata for a profile name."""
    pn = profile_name.lower().strip()
    if pn in _DEFAULT_GODS:
        return dict(_DEFAULT_GODS[pn])
    # Fallback: derive from profile name
    return {
        'display_name': pn.capitalize(),
        'icon': '🧑‍💻',
        'color': '#748FFC',
        'domain': 'Specialist',
    }
...
def _read_god_metadata(profile_dir: str | Path) -> dict:
    """Read god.json from a profile directory, or return defaults."""
    profile_path = Path(profile_dir) if isinstance(profile_dir, str) else profile_dir
    god_path = profile_path / _GOD_METADATA_FILE
    if god_path.exists():
        try:
            return json.loads(god_path.read_text(encoding='utf-8'))
        except (json.JSONDecodeError, OSError):
            logger.warning("Failed to parse %s, using defaults", god_path)
    # Fall back to name-based default
    # Root profile (~/.hermes itself) has dir name '.hermes' — map to 'default'
    name = profile_path.name
    try:
        if profile_path.resolve() == _DEFAULT_HERMES_HOME.resolve():
            name = 'default'
    except Exception:
        pass
    return _default_god_metadata(name)


def _write_god_metadata(profile_dir: str | Path, metadata: dict) -> dict:
    """Write god metadata to a profile directory's god.json. Returns the metadata."""
    profile_path = Path(profile_dir) if isinstance(profile_dir, str) else profile_dir
    god_path = profile_path / _GOD_METADATA_FILE
    god_path.write_text(
...
def _default_profile_dict() -> dict:
    ...
    return {
        ...
        'skill_count': 0,
        'god': _default_god_metadata('default'),
    }
...
def create_profile_api(name: str, clone_from: str = None,
    ...
    return {
        ...
        'skill_count': 0,
        'god': _read_god_metadata(profile_path),
    }
