...
function _workspacePanelEls(){
  return {
    ...
    collapseBtn: $('btnCollapseRightPanel'),
  };
}
...
// ── Mobile swipe-to-close right panel ─────────────────────────────────────
// Swipe right on the workspace/forge panel to close it.
(function() {
  var panel, startX, panelWidth, isDragging = false, offset = 0;

  function _getPanel() {
    if (!panel) panel = document.querySelector('.rightpanel');
  ...
  function onTouchMove(e) {
    ...
    var p = _getPanel();
    if (!p) { isDragging = false; return; }
    var dx = e.touches[0].clientX - startX;
    offset = Math.min(Math.max(0, dx), panelWidth);
    // Swipe right = shift panel right = closing direction
  ...
  function onTouchEnd() {
    ...
    p.style.transition = '';
    if (offset > panelWidth * 0.35) {
      // Past threshold — slide to fully closed, then cleanup
      p.style.transform = 'translateX(' + panelWidth + 'px)';
      setTimeout(function() {
        if (p) {
          p.style.transform = '';
          p.style.transition = '';
...
// ── Chromium mobile keyboard fix ──────────────────────────────────────────
// Chromium Android has a well-known bug where position:fixed elements don't
// re-render when the virtual keyboard opens — they stay anchored to the
// layout viewport instead of the visual viewport. This pushes the titlebar
// (and other fixed elements) off-screen.
//
// The fix: when the keyboard opens, freeze the body to the visual viewport
// dimensions with position:fixed. This forces Chromium to treat the visual
// viewport as the effective viewport for fixed positioning. On close,
// restore normal scrolling and resume position.
(function() {
  if (!window.visualViewport) return;

  var _kpBody = document.body;
  var _kpScrollY = 0;
  var _kpActive = false;
  var _kpTitlebar = document.querySelector('.app-titlebar');

  function _kpLock() {
    if (_kpActive) return;
    _kpActive = true;
    _kpScrollY = window.scrollY;
    // Freeze the body at the current scroll position and size it to the
    // visual viewport. This prevents Chromium from auto-scrolling and
    // forces fixed elements to use the visual viewport as their origin.
    _kpBody.style.position = 'fixed';
    _kpBody.style.top = '-' + _kpScrollY + 'px';
    _kpBody.style.left = '0';
    _kpBody.style.width = '100%';
    _kpBody.style.height = window.visualViewport.height + 'px';
    _kpBody.style.overflow = 'hidden';
    // Also keep the titlebar at visible top
    if (_kpTitlebar) {
      _kpTitlebar.style.top = window.visualViewport.offsetTop + 'px';
    }
  }
  ...
  function _kpUnlock() {
    if (!_kpActive) return;
    _kpActive = false;
    var prevScrollY = _kpScrollY;
    _kpBody.style.position = '';
    _kpBody.style.top = '';
    _kpBody.style.left = '';
    _kpBody.style.width = '';
    _kpBody.style.height = '';
    _kpBody.style.overflow = '';
    // Restore scroll position
    window.scrollTo(0, prevScrollY);
    // Reset titlebar position
    if (_kpTitlebar) {
      _kpTitlebar.style.top = '';
    }
  }

  function _kpOnViewportChange() {
    var vh = window.visualViewport.height;
    var lh = window.innerHeight;
    var isKeyboardOpen = vh < lh * 0.75;

    if (isKeyboardOpen && !_kpActive) {
      _kpLock();
    } else if (!isKeyboardOpen && _kpActive) {
      _kpUnlock();
    }

    // When locked, keep titlebar synced to visual viewport offset
    if (_kpActive && _kpTitlebar) {
      _kpTitlebar.style.top = window.visualViewport.offsetTop + 'px';
    }
  }

  window.visualViewport.addEventListener('resize', _kpOnViewportChange);
  ...
  // Expose for bfcache / pageshow reuse
  window._kpSync = function() { _kpOnViewportChange(); };

  // Listen on focus/blur of message input as fallback (Vivaldi quirk)
  var _kpMsg = document.getElementById('msg');
  if (_kpMsg) {
    _kpMsg.addEventListener('focus', function() { setTimeout(_kpOnViewportChange, 100); });
    _kpMsg.addEventListener('blur', function() { setTimeout(_kpOnViewportChange, 200); });
  }
})();

// ── Service worker update detection ──────────────────────────────────────
// PWAs have no refresh button, so users can get stuck on old cached versions.
// We silently check for updates, and when a new SW is found, show a toast
// banner so the user can tap to reload and get the latest version.
(function() {
  ...
  function _showUpdateToast() {
    ...
    toast.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);' +
      'background:var(--accent,#8b5cf6);color:#fff;padding:10px 20px;border-radius:10px;' +
      'font-size:13px;font-weight:600;z-index:9999;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.3);' +
    ...
    setTimeout(function() { toast.style.opacity = '0'; setTimeout(function() { toast.remove(); }, 400); }, 15000);
  }
  ...
  navigator.serviceWorker.getRegistration().then(function(reg) {
    ...
    reg.addEventListener('updatefound', function() {
      ...
      newSW.addEventListener('statechange', function() {
        // installed/waiting means the new SW is ready but page hasn't reloaded
        if (this.state === 'installed' || this.state === 'waiting') {
          _showUpdateToast();
        }
      });
    });
  ...
  // If a new SW takes over while the page is open, reload automatically
...
async function _updateProfileChip() {
  // Fetch active profile data to get god icon/display_name for the chip
  try {
    const data = await api('/api/profile/active');
    const name = data.name || 'default';
    if (typeof S !== 'undefined') S.activeProfile = name;
    const chip = document.getElementById('profileChip');
    const label = document.getElementById('profileChipLabel');
    if (!label) return;
    const god = data.god || {};
    ...
    if (chip) {
      chip.title = 'Switch god — ' + displayName;
      const iconEl = chip.querySelector('.composer-profile-icon');
    if (iconEl) {
        // Render <img> for URL icons, text for emoji
        if (typeof icon === 'string' && (icon.startsWith('http') || icon.startsWith('/api/'))) {
          iconEl.innerHTML = '<img src="' + icon.replace(/"/g,'&quot;') + '" alt="" style="width:100%;height:100%;object-fit:cover;display:block">';
        } else {
    ...
    const label = document.getElementById('profileChipLabel');
    if (label && typeof S !== 'undefined') label.textContent = S.activeProfile || 'default';
  }
}
...
(async()=>{
  ...
  try{
    ...
    if(window.PantheonTheme && typeof PantheonTheme.hydrate==='function'){
      PantheonTheme.hydrate({
        theme: appearance.theme,
        skin: appearance.skin,
        font_size: fontSize,
        icon_pack: s.icon_pack || 'pixel',
        layout_density: s.layout_density || 'comfortable',
        custom_theme: s.custom_theme || null,
      });
    }
    // Rewrite nav icons to match saved icon pack
    if(typeof _rewriteNavIcons==='function'){
      _rewriteNavIcons(s.icon_pack || 'pixel');
    }
  ...
  // Re-run applyBotName now that activeProfile is known, so sidebar header,
  // document title, and placeholder reflect the profile name instead of the global bot name.
  applyBotName();
  // Update profile chip label with god info
  _updateProfileChip();
...
window.addEventListener('pageshow', async (event) => {
  ...
  // Keyboard fix — consolidate with the global visualViewport fix above.
  // The global fix handles real-time scroll/resize tracking. We only need
  // a bfcache-specific one-shot reset here, plus a blur fallback that
  // delegates to the global fix's logic via _kpSync (defined globally below).
  if (window._kpSync) {
  }
  // Bfcache may restore with a stale titlebar position — sync once.
  setTimeout(window._kpSync, 50);
  // Blur fallback for when visualViewport.resize doesn't fire (Vivaldi).
  // This only runs on keyboard close, so no conflict with global scroll sync.
  var _kpMsgEl = document.getElementById('msg');
  if (_kpMsgEl) {
    _kpMsgEl.addEventListener('blur', function() {
      setTimeout(function() {
        if (window._kpSync) window._kpSync();
      }, 150);
    });
  }
});

// ── Phase 3: Icon pack picker ──────────────────────────────────────────────
function _pickIconPack(packName){
  if(!packName) return;
  const sel=$('settingsIconPack');
  if(sel) sel.value=packName;
  // Update icon pack preview
  const preview=$('iconPackPreview');
  if(preview){
    const imgs=preview.querySelectorAll('img');
    const names=['Communion_icon.png','athenaeum_icon.png','forge_icon.png','settings_icon.png'];
    imgs.forEach(function(img,i){
      if(i<names.length) img.src='static/icons/'+packName+'/'+names[i];
    });
  }
  // Rewrite nav rail and sidebar icon paths
  _rewriteNavIcons(packName);
  // Update SDK
  if(window.PantheonTheme && typeof PantheonTheme.setIconPack==='function'){
    PantheonTheme.setIconPack(packName);
  }
  _scheduleAppearanceAutosave();
}

/**
 * Rewrite all nav icon <img> src attributes to point to the active icon pack.
 * Targets .rail .nav-icon and .sidebar-nav .nav-icon elements.
 * Skips god rail icons (loaded dynamically from /api/gods).
 */
function _rewriteNavIcons(packName){
  if(!packName) return;
  document.querySelectorAll('.rail .nav-icon, .sidebar-nav .nav-icon').forEach(function(img){
    var src=img.getAttribute('src');
    if(!src) return;
    // Extract filename: handle both flat (static/icons/X.png) and pack-prefixed paths
    var match=src.match(/[^/]+\.(png|svg|webp|gif)(\?.*)?$/);
    if(!match) return;
    var filename=match[0].replace(/\?.*$/,'');
    var newSrc='static/icons/'+packName+'/'+filename;
    if(newSrc!==src){
...
// ── Phase 3: Layout density picker ──────────────────────────────────────────
function _pickDensity(density){
  if(!['compact','comfortable','spacious'].includes(density)) return;
  const hidden=$('settingsDensity');
  if(hidden) hidden.value=density;
  // Update button visuals
  document.querySelectorAll('#densityPickerGrid .density-pick-btn').forEach(function(btn){
    const val=btn.dataset.densityVal;
    if(val===density){
      btn.style.borderColor='var(--accent)';
      btn.style.background='var(--accent-bg)';
      btn.style.boxShadow='0 0 0 2px var(--accent-bg-strong)';
    }else{
  ...
  if(window.PantheonTheme && typeof PantheonTheme.setLayoutDensity==='function'){
    PantheonTheme.setLayoutDensity(density);
  }
  _scheduleAppearanceAutosave();
}

// ── Phase 3: Custom color change handler ────────────────────────────────────
function _onCustomColorChange(){
  // Update hex value displays
  const ids=[
    'customBgLight','customTextLight','customAccentLight','customSurfaceLight','customBorderLight','customMutedLight',
    'customBgDark','customTextDark','customAccentDark','customSurfaceDark','customBorderDark','customMutedDark'
  ];
  ...
  const customColors={
    light: {
      bg: $('#customBgLight')?.value||'#FEFCF7',
      text: $('#customTextLight')?.value||'#1A1610',
      accent: $('#customAccentLight')?.value||'#B8860B',
      surface: $('#customSurfaceLight')?.value||'#F3EEE3',
      border: $('#customBorderLight')?.value||'#E0D8C8',
      muted: $('#customMutedLight')?.value||'#5C5344',
    },
    dark: {
      bg: $('#customBgDark')?.value||'#0D0D1A',
      text: $('#customTextDark')?.value||'#FFF8DC',
      accent: $('#customAccentDark')?.value||'#FFD700',
      surface: $('#customSurfaceDark')?.value||'#1A1A2E',
      border: $('#customBorderDark')?.value||'#2A2A45',
      muted: $('#customMutedDark')?.value||'#C0C0C0',
    },
  };
  if(window.PantheonTheme && typeof PantheonTheme.setColors==='function'){
    PantheonTheme.setColors(customColors);
  }
  _scheduleAppearanceAutosave();
}

// ── Phase 3: Show/hide custom theme section based on selected theme ──────
// Extended version of _pickTheme that also handles custom mode
var _origPickTheme = window._pickTheme;
function _pickTheme(name){
  const customSection=$('customThemeSection');
  if(name==='custom'){
    if(customSection) customSection.style.display='';
    // Enable custom colors
    _onCustomColorChange();
  }else{
    if(customSection) customSection.style.display='none';
    // Clear custom colors when switching to a preset
    if(window.PantheonTheme && typeof PantheonTheme.setColors==='function'){
      PantheonTheme.setColors(null);
    }
  }
  // Call original handler
  if(typeof _origPickTheme==='function') _origPickTheme(name);
  else{
    // Fallback if _origPickTheme wasn't defined
