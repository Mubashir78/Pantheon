...
async function loadSession(sid){
  ...
  try {
    ...
    if(_msgInner){
      if(e.status===404){
        _msgInner.innerHTML='<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">Communion not available in web UI.</div>';
...
function renderSessionListFromCache(){
  ...
  // Group sessions by date (and by god when showing cross-profile)
  const isCrossProfile=Boolean(_showAllProfiles);
  const groups=[];
  let curGod=null, curGodName=null, curLabel=null, curItems=[];
  function _flushGodGroup(){
    if(curItems.length){
      const label=curGodName||'Unknown';
      groups.push({label:label,items:curItems,isGodGroup:true,isPinned:false});
      curItems=[];
    }
  }
  function _flushDateGroup(){
    if(curItems.length){
      groups.push({label:curLabel,items:curItems,isPinned:false});
      curItems=[];
    }
  }
  if(pinned.length) groups.push({label:'\u2605 Pinned',items:pinned,isPinned:true});
  const _godNameForProfile=(profile)=>{
    if(!profile) return null;
    const god=window._godMap&&_godMap[profile];
    return god?god.display_name:profile;
  };
  if(isCrossProfile){
    // Group by god first, then by date within each god
    const byGod={};
    for(const s of unpinned){
      const profile=s.profile||'default';
      if(!byGod[profile]) byGod[profile]=[];
      byGod[profile].push(s);
    }
    for(const [profile, godSessions] of Object.entries(byGod)){
      const godName=_godNameForProfile(profile);
      curGodName=godName; curLabel=null; curItems=[];
      godSessions.sort((a,b)=>_sessionTimestampMs(b)-_sessionTimestampMs(a));
      for(const s of godSessions){
      ...
      _flushDateGroup();
      // Mark the last N date-groups as belonging to this god
      for(let i=groups.length-1;i>=0;i--){
        const g=groups[i];
        if(g.isGodGroup||g.isPinned) break;
        g.godProfile=profile;
        g.godName=godName;
      }
    }
  } else {
...
async function deleteSession(sid){
  ...
  if(S.session&&S.session.session_id===sid){
    ...
    if(remaining.sessions&&remaining.sessions.length){
      ...
      const _tt=$('topbarTitle');if(_tt)_tt.textContent=(typeof S!=='undefined'&&S.activeProfile&&S.activeProfile!=='default')?S.activeProfile.charAt(0).toUpperCase()+S.activeProfile.slice(1):(window._botName||'Hermes');
