...
function syncAppTitlebar() {
  ...
  if (panel === 'chat' && typeof S !== 'undefined' && S && S.session) {
    ...
    if (S.session.is_cli_session) {
      ...
      subText = sourceLabel + (S.session.read_only ? ' · read-only' : '');
    }
  } else {
...
async function switchPanel(name, opts = {}) {
  ...
  if (mainEl) {
    ['settings','skills','memory','athenaeum','tasks','kanban','workspaces','profiles','insights','logs','gods'].forEach(p => {
  ...
  if (nextPanel === 'athenaeum') await loadAthenaeum();
  if (nextPanel === 'memory') await loadMemory();
  if (nextPanel === 'gods') await loadGodList();
...
function renderWorkspacesPanel(workspaces){
  ...
  for(let i=0;i<workspaces.length;i++){
    ...
    if(info) info.onclick = async (e) => { e.stopPropagation(); await switchToWorkspace(w.path, w.name); if(typeof switchPanel==='function') switchPanel('chat'); };
  ...
  // ── Inline "Add forge" row ──
  const addRow = document.createElement('div');
  addRow.className = 'ws-add-row';
  addRow.innerHTML = `
    <span class="ws-add-icon">${li('plus',14)}</span>
    <span class="ws-add-text" data-i18n="workspace_add_title">Add forge</span>
    <div class="ws-add-form" style="display:none">
      <input type="text" class="ws-add-input" placeholder="Workspace name" autocomplete="off" spellcheck="false">
      <button class="ws-add-submit" title="Create">${li('check',13)}</button>
      <button class="ws-add-cancel" title="Cancel">${li('x',13)}</button>
    </div>
  `;
  const addIcon = addRow.querySelector('.ws-add-icon');
  const addText = addRow.querySelector('.ws-add-text');
  const addForm = addRow.querySelector('.ws-add-form');
  const addInput = addRow.querySelector('.ws-add-input');
  const addSubmit = addRow.querySelector('.ws-add-submit');
  const addCancel = addRow.querySelector('.ws-add-cancel');
  ...
  addRow.onclick = (e) => { if(e.target.closest('.ws-add-form')) return; _showAddForm(); };
  addCancel.onclick = (e) => { e.stopPropagation(); _hideAddForm(); };
  addInput.onkeydown = (e) => {
    if(e.key === 'Enter'){ e.preventDefault(); _submitForgeCreate(addInput.value.trim()); }
    if(e.key === 'Escape'){ e.preventDefault(); _hideAddForm(); }
  };
  addSubmit.onclick = (e) => { e.stopPropagation(); _submitForgeCreate(addInput.value.trim()); };
  async function _submitForgeCreate(name){
    if(!name){ showToast('Name is required'); return; }
    addInput.disabled = true;
    try {
      const forgePath = '~/forge/' + encodeURIComponent(name).replace(/%20/g,' ');
      const data = await api('/api/workspaces/add', {
        method:'POST',
        body: JSON.stringify({ path: forgePath, name: name, create: true })
      });
      _workspaceList = data.workspaces || [];
      // Find the newly created workspace to get its resolved path
      const created = _workspaceList.find(w => w.name === name) || _workspaceList[_workspaceList.length - 1];
      showToast('Created forge: ' + name);
      _hideAddForm();
      renderWorkspacesPanel(_workspaceList);
      if(created){ await switchToWorkspace(created.path, created.name); if(typeof switchPanel==='function') switchPanel('chat'); }
    } catch(e){
...
// ── Header + button shortcut to open inline forge form ──
function openForgeCreate(){
  const panel = $('workspacesPanel');
  if(!panel) return;
  // Ensure we're on the workspaces panel
  if(typeof switchPanel === 'function') switchPanel('workspaces');
  // Find the add row and trigger its form
  const addRow = panel.querySelector('.ws-add-row');
  if(addRow){
    const form = addRow.querySelector('.ws-add-form');
    const icon = addRow.querySelector('.ws-add-icon');
    const text = addRow.querySelector('.ws-add-text');
    if(form && icon && text){
      icon.style.display = 'none';
      text.style.display = 'none';
      form.style.display = 'flex';
      const input = addRow.querySelector('.ws-add-input');
...
async function promptWorkspacePath(){
  ...
  try{
    ...
    if(String(e.message||'').includes('Workspace already in list') || String(e.message||'').includes('Forge already in list')){
...
function _renderProfileDetail(p, activeName){
  ...
  body.innerHTML = `
    <div class="main-view-content">
      <div class="detail-card">
        <div class="detail-card-title">God</div>
        ${rows.join('')}
      </div>
    </div>`;
  body.style.display = '';
...
function renderProfileDropdown(data) {
  ...
  for (const p of profiles) {
    ...
    const god = p.god || {};
    const icon = god.icon || '🧑‍ὋB';
    const color = god.color || '#748FFC';
    // Render <img> for URL icons, text for emoji
    var iconHtml;
    if (typeof icon === 'string' && (icon.startsWith('http') || icon.startsWith('/api/'))) {
      iconHtml = '<img src="' + icon.replace(/"/g,'&quot;') + '" style="width:100%;height:100%;object-fit:cover;display:block" alt="">';
    } else {
      iconHtml = icon || '🧑‍💻';
    }
    const displayName = god.display_name || p.name.charAt(0).toUpperCase() + p.name.slice(1);
    const meta = [];
    if (p.model) meta.push(p.model.split('/').pop());
    if (p.skill_count) meta.push(t('profile_skill_count', p.skill_count));
    ...
    opt.innerHTML = `<div class="profile-opt-name"><span class="profile-opt-icon" style="background:${color}22;overflow:hidden">${iconHtml}</span>${gwDot}${esc(displayName)}${defaultBadge}${checkmark}</div>` +
...
async function switchToProfile(name) {
  ...
  if (_chipLabel) {
    // Try to show god display name optimistically from cached profiles
    const cached = typeof _profilesCache !== 'undefined' && _profilesCache;
    let displayName = null;
    if (cached && cached.profiles) {
      const match = cached.profiles.find(p => p.name === name);
      if (match && match.god && match.god.display_name) displayName = match.god.display_name;
    }
    _chipLabel.textContent = displayName || name.charAt(0).toUpperCase() + name.slice(1);
  }
  ...
  try {
    ...
    // ── Refresh profile chip in both branches ──────────────────────────────
    if (typeof _updateProfileChip === 'function') _updateProfileChip();
...
let _settingsIconPackOnOpen = null; // track icon pack at open time for discard revert
let _settingsDensityOnOpen = null; // track density at open time for discard revert
...
function _appearancePayloadFromUi(){
  return {
    ...
    icon_pack: ($('settingsIconPack')||{}).value || 'pixel',
    layout_density: ($('settingsDensity')||{}).value || 'comfortable',
    custom_theme: _buildCustomThemePayload(),
  };
}
function _buildCustomThemePayload(){
  const el=v=>$(v)?.value||null;
  const light={bg:el('customBgLight'),text:el('customTextLight'),accent:el('customAccentLight'),surface:el('customSurfaceLight'),border:el('customBorderLight'),muted:el('customMutedLight')};
  const dark={bg:el('customBgDark'),text:el('customTextDark'),accent:el('customAccentDark'),surface:el('customSurfaceDark'),border:el('customBorderDark'),muted:el('customMutedDark')};
  const hasLight=Object.values(light).some(v=>v!==null);
  const hasDark=Object.values(dark).some(v=>v!==null);
  if(!hasLight&&!hasDark) return {};
  const result={};
  if(hasLight) result.light=light;
  if(hasDark) result.dark=dark;
  return result;
}
...
function _rememberAppearanceSaved(payload){
  ...
  _settingsIconPackOnOpen=payload.icon_pack||'pixel';
  _settingsDensityOnOpen=payload.layout_density||'comfortable';
}
...
async function loadSettingsPanel(){
  try{
    ...
    // Hydrate icon pack dropdown
    const iconPackVal=settings.icon_pack||'pixel';
    const iconPackSel=$('settingsIconPack');
    if(iconPackSel) iconPackSel.value=iconPackVal;
    // Hydrate layout density selector
    const densityVal=settings.layout_density||'comfortable';
    if(typeof _pickDensity==='function') _pickDensity(densityVal);
    // Hydrate custom theme color pickers (only if custom_theme has data)
    if(settings.custom_theme && typeof settings.custom_theme==='object'){
      const ct=settings.custom_theme;
      // Light mode color pickers
      if(ct.light){
        const l=ct.light;
        const lightMap={bg:'customBgLight',text:'customTextLight',accent:'customAccentLight',surface:'customSurfaceLight',border:'customBorderLight',muted:'customMutedLight'};
        for(const [key,id] of Object.entries(lightMap)){
          const el=$(id);
          if(el && l[key]){ el.value=l[key]; const valEl=$(id+'Val'); if(valEl) valEl.textContent=l[key]; }
        }
      }
      // Dark mode color pickers
      if(ct.dark){
        const d=ct.dark;
        const darkMap={bg:'customBgDark',text:'customTextDark',accent:'customAccentDark',surface:'customSurfaceDark',border:'customBorderDark',muted:'customMutedDark'};
        for(const [key,id] of Object.entries(darkMap)){
          const el=$(id);
          if(el && d[key]){ el.value=d[key]; const valEl=$(id+'Val'); if(valEl) valEl.textContent=d[key]; }
        }
      }
    }
    // Hydrate PantheonTheme SDK with loaded settings
    if(window.PantheonTheme && typeof PantheonTheme.hydrate==='function'){
      PantheonTheme.hydrate(settings);
    }
...
/** Safe HTML escaping — prevents XSS in rendered user/knowledge content */
function escHtml(s) {
  return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');
}

// ── Athenaeum ─────────────────────────────────────────────────────────────
// Knowledge codex browser + semantic search

let _athenaeumStack = [];       // Directory history stack
let _athenaeumCurrentPath = ''; // Current directory being browsed
let _athenaeumCurrentFile = ''; // Currently viewed file path
let _athenaeumSearchTimer = null;

/** Build an HTML breadcrumb from a directory or file path.
 *  Each segment before the last is clickable to navigate there directly.
 *  The last segment is plain text (current location).
 */
function _athenaeumRenderBreadcrumb(dirPath, filePath) {
  // At root (no path)
  if (!dirPath && !filePath) {
    return '<span style="font-weight:600;font-size:14px">Athenaeum</span>';
  }
  const path = filePath || dirPath;
  // Strip INDEX.md suffix if present
  const cleanPath = path.endsWith('/INDEX.md') ? path.slice(0, -9) : path;
  const segments = cleanPath.split('/').filter(Boolean);
  if (segments.length === 0) return '<span style="font-weight:600;font-size:14px">Athenaeum</span>';

  let html = '<span class="athenaeum-breadcrumb" style="display:flex;align-items:center;gap:4px;font-size:13px;flex-wrap:wrap">';
  // Root link
  html += '<a href="#" onclick="event.preventDefault();_athenaeumGoTo(-1)" style="color:var(--accent);text-decoration:none;cursor:pointer;white-space:nowrap" title="Go to root">Athenaeum</a>';
  ...
  segments.forEach((seg, i) => {
    ...
    if (isLast) {
      const icon = filePath ? (seg.endsWith('.md') ? '📄' : '📎') : '📁';
      html += `<span style="font-weight:600;white-space:nowrap">${icon} ${escHtml(displayName)}</span>`;
    } else {
      html += `<a href="#" onclick="event.preventDefault();_athenaeumGoTo(${i})" style="color:var(--accent);text-decoration:none;cursor:pointer;white-space:nowrap">${escHtml(displayName)}</a>`;
      html += '<span style="color:var(--muted);font-size:10px">›</span>';
    }
  });
  html += '</span>';
 ...
 *  Index 0 = first subdirectory after Athenaeum (e.g. Codex-Forge).
 *  Index -1 = root.
 *  Rebuilds the stack so forward history beyond that point is discarded.
 */
function _athenaeumGoTo(idx) {
  if (idx === -1) {
    // Root
    _athenaeumStack = [];
    _athenaeumCurrentPath = '';
    _athenaeumCurrentFile = '';
    loadAthenaeum();
    return;
  }
  // Reconstruct the path up to this segment
  const currentPath = _athenaeumCurrentFile || _athenaeumCurrentPath;
  const cleanPath = currentPath.endsWith('/INDEX.md') ? currentPath.slice(0, -9) : currentPath;
  const segments = cleanPath.split('/').filter(Boolean);
  if (idx >= segments.length) return;

  // Build path up to this segment
  const targetDir = segments.slice(0, idx + 1).join('/') + '/INDEX.md';
  // Rebuild stack: each parent level before this
  _athenaeumStack = [];
  for (let i = 0; i < idx; i++) {
    _athenaeumStack.push(segments.slice(0, i + 1).join('/') + '/INDEX.md');
  }
  _athenaeumCurrentPath = targetDir;
  _athenaeumCurrentFile = '';
  _athenaeumLoadDirectory(targetDir);
}

/** Load the Athenaeum panel — show codex list */
async function loadAthenaeum() {
  const panel = $('athenaeumPanel');
  const loading = $('athenaeumLoading');
  if (!panel) return;
  // Reset state
  _athenaeumStack = [];
  _athenaeumCurrentPath = '';
  ...
  try {
    const data = await api('/api/athenaeum/list?details=true');
    if (!data || !data.codexes) {
      panel.innerHTML = '<div style="padding:12px;color:var(--muted);font-size:12px">Failed to load Athenaeum</div>';
      return;
    }
    const codexes = data.codexes;
    if (!codexes.length) {
      panel.innerHTML = '<div style="padding:12px;color:var(--muted);font-size:12px">No codexes found</div>';
      return;
    }
    let html = '';
    codexes.forEach(c => {
      const name = c.name || c;
      const count = c.file_count !== undefined ? ` (${c.file_count})` : '';
      html += `<button type="button" class="side-menu-item" onclick="athenaeumBrowse('${escHtml(name)}/INDEX.md')" style="display:flex;align-items:center;gap:8px">
        <span>📚</span><span style="flex:1">${escHtml(name)}${count}</span><span style="color:var(--muted);font-size:11px">›</span>
      </button>`;
    });
    panel.innerHTML = html;
    // Show empty state in main view
    _athenaeumShowEmpty();
  } catch (e) {
    panel.innerHTML = `<div style="padding:12px;color:var(--err);font-size:12px">Error: ${escHtml(e.message)}</div>`;
  }
}

/** Browse a directory path */
async function athenaeumBrowse(path) {
  if (!path) return;
  // Push current path onto stack if not already at root
  if (_athenaeumCurrentPath) _athenaeumStack.push(_athenaeumCurrentPath);
  _athenaeumCurrentPath = path;
  _athenaeumCurrentFile = '';
  await _athenaeumLoadDirectory(path);
}

/** Navigate up one directory level */
function athenaeumGoUp() {
  if (_athenaeumStack.length === 0) {
    // At root — reload codex list
    _athenaeumStack = [];
    _athenaeumCurrentPath = '';
    loadAthenaeum();
    return;
  }
  const prevPath = _athenaeumStack.pop();
  _athenaeumCurrentPath = prevPath;
  if (prevPath === 'INDEX.md') {
    // Back to root
    loadAthenaeum();
    return;
  }
  _athenaeumLoadDirectory(prevPath);
}

/** Refresh current view */
async function athenaeumRefresh() {
  if (_athenaeumCurrentFile) {
    await athenaeumOpenFile(_athenaeumCurrentFile);
  } else if (_athenaeumCurrentPath) {
    await _athenaeumLoadDirectory(_athenaeumCurrentPath);
  } else {
    await loadAthenaeum();
  }
}
...
function athenaeumSearchInput(value) {
  clearTimeout(_athenaeumSearchTimer);
  if (!value || value.length < 2) {
    // Clear: reload codex list
    if (!value) loadAthenaeum();
    return;
  }
  _athenaeumSearchTimer = setTimeout(() => athenaeumSearch(value), 400);
}

/** Execute semantic search */
async function athenaeumSearch(query) {
  const panel = $('athenaeumPanel');
  if (!panel) return;
  const title = $('athenaeumDetailTitle');
  const body = $('athenaeumDetailBody');
  const empty = $('athenaeumDetailEmpty');
  const backBtn = $('btnAthenaeumBack');

  if (title) title.innerHTML = '<span style="font-weight:600;font-size:14px">🔍 Searching...</span>';
  if (empty) empty.style.display = 'none';
  if (body) body.style.display = 'block';
  if (backBtn) backBtn.style.display = 'inline-flex';
  _athenaeumCurrentFile = '';
  _athenaeumStack.push(_athenaeumCurrentPath);
  _athenaeumCurrentPath = '__search__';

  try {
    const data = await api('/api/athenaeum/search?q=' + encodeURIComponent(query) + '&n=10');
    // Sidebar: show search results
    if (data && data.results && data.results.length) {
      let html = '<div style="padding:8px 12px;font-size:11px;color:var(--accent)">Search: ' + escHtml(query) + '</div>';
      data.results.forEach(r => {
        html += `<button type="button" class="side-menu-item" onclick="athenaeumOpenFile('${escHtml(r.source)}')" style="padding:6px 12px;flex-direction:column;align-items:flex-start;gap:2px">
          <span style="font-size:12px;font-weight:500">${escHtml(r.codex)}</span>
          <span style="font-size:11px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%">${escHtml(r.source.split('/').pop())}</span>
          <span style="font-size:10px;color:var(--accent)">${Math.round(r.score * 100)}% match</span>
        </button>`;
      });
      panel.innerHTML = html;
    } else {
      panel.innerHTML = '<div style="padding:12px;color:var(--muted);font-size:12px">No results found</div>';
    }
    // Main view: results detail
    if (title) title.innerHTML = '<span style="font-weight:600;font-size:14px">🔍 <span style="color:var(--accent)">' + escHtml(query) + '</span></span>';
    if (body) {
      if (data && data.results && data.results.length) {
        let h = '<div style="padding:16px;max-width:800px">';
        h += '<p style="color:var(--muted);margin-bottom:12px">' + data.total + ' results across ' + data.codexes_searched + ' codexes</p>';
        data.results.forEach((r, i) => {
          h += '<div class="side-menu-item" style="cursor:pointer;margin-bottom:8px;border-radius:8px;padding:12px" onclick="athenaeumOpenFile(\'' + escHtml(r.source) + '\')">';
          h += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">';
          h += '<strong style="font-size:13px">' + escHtml(r.codex) + '</strong>';
          h += '<span style="font-size:11px;color:var(--accent)">' + Math.round(r.score * 100) + '%</span>';
          h += '</div>';
          h += '<div style="font-size:11px;color:var(--muted);margin-bottom:6px">' + escHtml(r.source) + '</div>';
          h += '<div style="font-size:12px;line-height:1.5;max-height:60px;overflow:hidden;text-overflow:ellipsis">' + escHtml(r.content.substring(0, 300)) + '</div>';
          h += '</div>';
        });
        h += '</div>';
        body.innerHTML = h;
      } else {
        body.innerHTML = '<div style="padding:24px;color:var(--muted);font-size:13px">No results found for "' + escHtml(query) + '"</div>';
      }
    }
  } catch (e) {
    panel.innerHTML = '<div style="padding:12px;color:var(--err);font-size:12px">Search error: ' + escHtml(e.message) + '</div>';
  }
}

/** Open and view a file */
async function athenaeumOpenFile(path) {
  if (!path) return;
  _athenaeumCurrentFile = path;
  const title = $('athenaeumDetailTitle');
  const body = $('athenaeumDetailBody');
  const empty = $('athenaeumDetailEmpty');
  const backBtn = $('btnAthenaeumBack');
  ...
  if (title) title.innerHTML = '<span style="font-weight:600;font-size:14px">⌛ Loading...</span>';
  if (empty) empty.style.display = 'none';
  if (body) body.style.display = 'block';
  if (backBtn) backBtn.style.display = 'inline-flex';
  ...
  if (panel) {
    panel.querySelectorAll('.side-menu-item').forEach(el => el.classList.remove('active'));
    panel.querySelectorAll('[onclick*="' + path.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '"]').forEach(el => el.classList.add('active'));
  }

  try {
    const data = await api('/api/athenaeum/read?path=' + encodeURIComponent(path));
    if (!data || data.error) {
      if (title) title.innerHTML = '<span style="font-weight:600;font-size:14px;color:var(--err)">⚠ Error</span>';
      if (body) body.innerHTML = '<div style="padding:24px;color:var(--err)">' + escHtml(data?.error || 'Failed to load') + '</div>';
      return;
    }
    if (title) title.innerHTML = _athenaeumRenderBreadcrumb('', path);
    const sub = path.split('/').slice(0, -1).join('/') || 'athenaeum';
    if (body) {
      const name = path.split('/').pop();
      const ext = name.includes('.') ? name.split('.').pop() : '';
      const isMarkdown = ['md', 'mdx', 'markdown'].includes(ext);
      let html = '<div style="padding:16px;max-width:900px">';
      ...
      html += '<span style="font-size:11px;color:var(--muted)">' + escHtml(path) + '</span>';
      html += '<span style="font-size:11px;color:var(--muted)">' + data.total_lines + ' lines · ' + data.size_kb + ' KB</span>';
      html += '</div>';
      if (isMarkdown) {
        html += '<div class="athenaeum-md-content" style="line-height:1.7;font-size:14px">' + escHtml(data.content) + '</div>';
      } else {
        html += '<pre style="background:var(--bg2);padding:12px;border-radius:8px;overflow:auto;font-size:12px;line-height:1.5;max-height:70vh">' + escHtml(data.content) + '</pre>';
      }
      html += '</div>';
      body.innerHTML = html;
    }
  } catch (e) {
    if (body) body.innerHTML = '<div style="padding:24px;color:var(--err)">Error: ' + escHtml(e.message) + '</div>';
  }
}

/** Load and render a directory listing */
async function _athenaeumLoadDirectory(dirPath) {
  const panel = $('athenaeumPanel');
  const title = $('athenaeumDetailTitle');
  const body = $('athenaeumDetailBody');
  const empty = $('athenaeumDetailEmpty');
  ...
  try {
    const data = await api('/api/athenaeum/walk?path=' + encodeURIComponent(dirPath));
    if (!data || data.error) {
      if (panel) panel.innerHTML = '<div style="padding:12px;color:var(--err);font-size:12px">' + escHtml(data?.error || 'Failed to load') + '</div>';
      return;
    }

    const dirName = data.directory === '.' ? 'Athenaeum' : data.directory;
    if (title) title.innerHTML = _athenaeumRenderBreadcrumb(dirPath, '');

    // Render sidebar: subdirs + files at this level
    if (panel) {
      let html = '';
      if (data.subdirectories && data.subdirectories.length) {
        data.subdirectories.forEach(d => {
          html += `<button type="button" class="side-menu-item" onclick="athenaeumBrowse('${escHtml(d.path)}/INDEX.md')" style="display:flex;align-items:center;gap:8px">
            <span>📁</span><span style="flex:1">${escHtml(d.name)}</span><span style="color:var(--muted);font-size:11px">›</span>
          </button>`;
        });
      }
      if (data.files && data.files.length) {
        data.files.forEach(f => {
          ...
          html += `<button type="button" class="side-menu-item" onclick="athenaeumOpenFile('${escHtml(f.path)}')" style="display:flex;align-items:center;gap:8px">
            <span>${icon}</span><span style="flex:1">${escHtml(f.name)}</span><span style="color:var(--muted);font-size:11px">${f.size_kb}KB</span>
          </button>`;
        });
      }
      if (!html) {
        html = '<div style="padding:12px;color:var(--muted);font-size:12px">(empty directory)</div>';
      }
      panel.innerHTML = html;
    }

    // Main view: show INDEX.md content + directory listing
    if (body) {
      let h = '<div style="padding:16px;max-width:900px">';
      // Show INDEX.md content if available
      if (data.index_content && data.index_content !== '(no INDEX.md found)') {
        h += '<div style="background:var(--bg2);border-radius:8px;padding:12px;margin-bottom:16px;font-size:13px;line-height:1.6;max-height:300px;overflow:auto">';
        h += '<div style="font-size:11px;color:var(--accent);margin-bottom:8px;text-transform:uppercase;letter-spacing:0.5px">INDEX.md</div>';
        h += escHtml(data.index_content);
        h += '</div>';
      }
      // Subdirectories
      if (data.subdirectories && data.subdirectories.length) {
        h += '<div style="margin-bottom:12px"><strong style="font-size:13px">Subdirectories</strong></div>';
        h += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-bottom:16px">';
        data.subdirectories.forEach(d => {
          h += `<div class="side-menu-item" style="cursor:pointer;padding:10px;border-radius:8px;display:flex;align-items:center;gap:8px" onclick="athenaeumBrowse('${escHtml(d.path)}/INDEX.md')">`;
          h += '📁 <span>' + escHtml(d.name) + '</span>';
          h += '</div>';
        });
        h += '</div>';
      }
      // Files
      if (data.files && data.files.length) {
        h += '<div style="margin-bottom:12px"><strong style="font-size:13px">Files</strong></div>';
        h += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-bottom:16px">';
        data.files.forEach(f => {
          const icon = f.name.endsWith('.md') ? '📄' : '📎';
          h += `<div class="side-menu-item" style="cursor:pointer;padding:10px;border-radius:8px;display:flex;align-items:center;gap:8px" onclick="athenaeumOpenFile('${escHtml(f.path)}')">`;
          h += icon + ' <span>' + escHtml(f.name) + '</span>';
          h += '<span style="margin-left:auto;color:var(--muted);font-size:11px">' + f.size_kb + 'KB</span>';
          h += '</div>';
        });
        h += '</div>';
      }
      if (!data.subdirectories?.length && !data.files?.length) {
        h += '<div style="color:var(--muted);font-size:13px;padding:24px;text-align:center">(empty directory)</div>';
      }
      h += '</div>';
      body.innerHTML = h;
    }
  } catch (e) {
    if (panel) panel.innerHTML = '<div style="padding:12px;color:var(--err);font-size:12px">Error: ' + escHtml(e.message) + '</div>';
  }
}

/** Show empty state in main view */
function _athenaeumShowEmpty() {
  const title = $('athenaeumDetailTitle');
  const body = $('athenaeumDetailBody');
  const empty = $('athenaeumDetailEmpty');
  const backBtn = $('btnAthenaeumBack');
  if (title) title.innerHTML = _athenaeumRenderBreadcrumb('', '');
...
// ── God Management ──────────────────────────────────────────────────────
let _editingGodName = null;
let _pendingGodIconData = null;

async function loadGodList() {
  const panel = $('godsPanel');
  if (!panel) return;
  panel.innerHTML = '<div style="padding:12px;color:var(--muted);font-size:12px">Loading...</div>';
  try {
    const r = await fetch('/api/gods');
    const d = await r.json();
    if (!d.gods || !d.gods.length) {
      panel.innerHTML = '<div style="padding:20px;color:var(--muted);font-size:13px;text-align:center">No gods yet.<br><button class="btn secondary" style="margin-top:8px" onclick="openGodEditor(\'__new__\')">Create one</button></div>';
      return;
    }
    panel.innerHTML = d.gods.map(g => {
      const nm = escHtml(g.name);
      const dn = escHtml(g.display_name || g.name);
      const dm = escHtml(g.domain || '');
      const cl = g.color || '#748FFC';
      let iconHtml = g.icon;
      if (g.icon && (g.icon.startsWith('http') || g.icon.startsWith('/api/'))) {
        iconHtml = `<img src="${escHtml(g.icon)}" style="width:100%;height:100%;object-fit:cover" alt="">`;
      } else if (!g.icon || g.icon === '🧑‍💻') {
        iconHtml = '<span style="font-size:16px">🧑‍💻</span>';
      } else {
        iconHtml = `<span style="font-size:16px">${g.icon}</span>`;
      }
      return `<button type="button" class="god-list-card" onclick="openGodEditor('${nm}')" style="display:flex;align-items:center;gap:10px;width:100%;padding:10px 12px;border:none;background:none;cursor:pointer;border-radius:8px;transition:background .15s;text-align:left;color:var(--text)">
        <div class="god-list-icon" style="width:36px;height:36px;border-radius:50%;background:${cl};display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden">${iconHtml}</div>
        <div style="flex:1;min-width:0">
          <div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${dn}</div>
          <div style="font-size:11px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${dm}</div>
        </div>
        ${g.is_active ? '<span style="font-size:10px;color:var(--accent);font-weight:600">ACTIVE</span>' : ''}
      </button>`;
    }).join('');
  } catch (e) {
    panel.innerHTML = `<div style="padding:12px;color:var(--error,#e05);font-size:12px">Failed to load: ${escHtml(e.message)}</div>`;
  }
}

async function openGodEditor(name) {
  // Close mobile sidebar drawer so the editor is visible underneath
  if (typeof closeMobileSidebar === 'function') closeMobileSidebar();
  _editingGodName = name === '__new__' ? null : name;
  _pendingGodIconData = null;
  const isNew = !_editingGodName;
  const title = $('godEditorTitle');
  const body = $('godEditorBody');
  const empty = $('godEditorEmpty');
  const saveBtn = $('btnSaveGodEditor');
  const delBtn = $('btnDeleteGodEditor');
  if (title) title.innerText = isNew ? 'New God' : `Edit: ${name}`;
  if (body) body.style.display = 'flex';
  if (empty) empty.style.display = 'none';
  if (saveBtn) saveBtn.style.display = 'inline-flex';
  if (delBtn) delBtn.style.display = isNew ? 'none' : 'inline-flex';

  if (body) body.innerHTML = '<div style="padding:20px;color:var(--muted)">Loading...</div>';

  try {
    const [providersResp, godsResp] = await Promise.all([
      fetch('/api/providers'),
      isNew ? Promise.resolve(null) : fetch('/api/gods/' + encodeURIComponent(name))
    ]);
    const providers = (await providersResp.json()).providers || [];
    let god = { name: '', display_name: '', domain: '', color: '#748FFC', icon: '', model: '', provider: '' };
    let soul = null;
    if (!isNew && godsResp) {
      const godData = await godsResp.json();
      god = { ...god, ...godData };
      try {
        const soulResp = await fetch('/api/gods/' + encodeURIComponent(name) + '/soul');
        if (soulResp.ok) {
          const soulData = await soulResp.json();
          soul = soulData.exists ? soulData.content : null;
        }
      } catch (_) {}
    }
    _renderGodEditorForm(body, god, soul, providers);
  } catch (e) {
    if (body) body.innerHTML = `<div style="padding:20px;color:var(--error,#e05)">Error: ${escHtml(e.message)}</div>`;
  }
}

function _renderGodEditorForm(container, god, soul, providers) {
  // Build provider/model dropdowns
  const providerOpts = providers.map(p =>
    `<option value="${escHtml(p.id)}" ${p.id === god.provider ? 'selected' : ''}>${escHtml(p.display_name || p.id)}</option>`
  ).join('');

  // Color swatches
  const SWATCHES = ['#F5C542','#FF6B6B','#E8590C','#FFD43B','#69DB7C','#748FFC','#DA77F2','#FF8787','#20C997','#845EF7','#F783AC','#4DABF7','#51CF66','#FCC419','#9775FA','#FF922B'];

  // Icon preview
  const iconUrl = god.icon && (god.icon.startsWith('/api/') || god.icon.startsWith('http'))
    ? god.icon : null;

  container.innerHTML = `
  <div class="god-editor-scroll">
    <div class="god-editor-layout">

      <!-- Left column: icon + basics -->
      <div class="god-editor-left">
        <div class="god-editor-icon-section">
          <div class="god-editor-icon-preview" id="godIconPreview" style="width:96px;height:96px;border-radius:50%;background:${god.color};display:flex;align-items:center;justify-content:center;overflow:hidden;font-size:36px;border:3px solid var(--border2);margin:0 auto 12px">
            ${iconUrl ? `<img src="${iconUrl}" style="width:100%;height:100%;object-fit:cover">` : (god.icon || '🧑‍💻')}
          </div>
          <label class="btn secondary" style="display:inline-flex;align-items:center;gap:6px;cursor:pointer;font-size:12px;padding:6px 14px;border-radius:8px">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
            Upload Icon
            <input type="file" accept="image/*" style="display:none" onchange="_handleGodIconUpload(this)">
          </label>
          <div style="font-size:10px;color:var(--muted);margin-top:6px;text-align:center">PNG, JPG, WebP • circle-cropped</div>
        </div>
      </div>

      <!-- Right column: fields -->
      <div class="god-editor-fields">
        <div class="god-editor-field-row">
          <label class="god-editor-label">Display Name</label>
          <input class="god-editor-input" id="godEditName" value="${escHtml(god.display_name || god.name)}" placeholder="God's display name">
        </div>

        <div class="god-editor-field-row">
          <label class="god-editor-label">Domain / Purpose</label>
          <textarea class="god-editor-input god-editor-textarea" id="godEditDomain" rows="2" placeholder="e.g. Sun · Song · Poetry">${escHtml(god.domain || '')}</textarea>
        </div>

        <div class="god-editor-field-row" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div>
            <label class="god-editor-label">Provider</label>
            <select class="god-editor-input god-editor-select" id="godEditProvider" onchange="_godEditorFillProviderModels()">
              <option value="">— Select —</option>
              ${providerOpts}
            </select>
          </div>
          <div>
            <label class="god-editor-label">Model</label>
            <select class="god-editor-input god-editor-select" id="godEditModel">
              <option value="">— Select —</option>
            </select>
          </div>
        </div>
        ...
        <div class="god-editor-field-row">
          <label class="god-editor-label">Color</label>
          <div style="display:flex;gap:8px;align-items:center">
            <div class="god-editor-color-grid">
              ${SWATCHES.map(c => `
                <button type="button" class="god-editor-swatch ${c === god.color ? 'active' : ''}" style="background:${c}" data-color="${c}" onclick="document.getElementById('godEditColor').value='${c}';this.parentElement.querySelectorAll('.god-editor-swatch').forEach(s=>s.classList.remove('active'));this.classList.add('active');document.getElementById('godIconPreview').style.background='${c}'"></button>
              `).join('')}
            </div>
            <input class="god-editor-input god-editor-color-input" id="godEditColor" value="${god.color}" placeholder="#HEX" style="width:80px;font-family:mono" oninput="document.getElementById('godIconPreview').style.background=this.value">
          </div>
        </div>

        <div class="god-editor-field-row">
          <label class="god-editor-label">SOUL.md</label>
          <div style="margin-bottom:6px;display:flex;gap:6px;flex-wrap:wrap;align-items:center">
            <button class="btn secondary" onclick="openForgePopup('${escHtml(god.name)}')" style="font-size:11px;padding:4px 10px">🔥 Forge This God's Soul</button>
            <button class="btn secondary" onclick="openExportModal('${escHtml(god.name)}')" style="font-size:11px;padding:4px 10px">📦 Export Bundle</button>
            ${soul === null ? '<span style="font-size:11px;color:var(--muted)">This god does not have a SOUL.md yet.</span>' : ''}
          </div>
          <textarea class="god-editor-input god-editor-textarea god-editor-soul-editor" id="godEditSoul" rows="16" spellcheck="false" placeholder="# ${escHtml(god.display_name || god.name)} &mdash; Title&#10;&#10;## Identity&#10;You are...">${soul !== null ? escHtml(soul) : ''}</textarea>
        </div>
      </div>
    </div>
  </div>`;

  // Fill models for the selected provider
  _godEditorFillProviderModels();
}

function _godEditorFillProviderModels() {
  const provSelect = document.getElementById('godEditProvider');
  const modelSelect = document.getElementById('godEditModel');
  if (!provSelect || !modelSelect) return;
  const selectedProvider = provSelect.value;
  modelSelect.innerHTML = '<option value="">— Select —</option>';
  if (!selectedProvider) return;
  // Find provider in the already-loaded data
  // We need to fetch providers to get model list
  fetch('/api/providers').then(r => r.json()).then(d => {
    const prov = (d.providers || []).find(p => p.id === selectedProvider);
    if (prov && prov.models) {
      prov.models.forEach(m => {
        const mid = m.id || m;
        const mlab = m.label || m.id || m;
        const selected = mid === (window._godEditorModel || '') ? 'selected' : '';
        modelSelect.innerHTML += `<option value="${escHtml(mid)}" ${selected}>${escHtml(mlab)}</option>`;
      });
    }
  }).catch(() => {});
}

function _handleGodIconUpload(input) {
  if (!input || !input.files || !input.files[0]) return;
  const file = input.files[0];
  const reader = new FileReader();
  reader.onload = function(e) {
    _pendingGodIconData = e.target.result;
...
async function saveGodEditor() {
  const isNew = !_editingGodName;
  const name = _editingGodName || (document.getElementById('godEditName')?.value || '').trim().toLowerCase().replace(/[^a-z0-9_-]/g, '-').replace(/^-+|-+$/g, '') || 'new-god';

  if (isNew && !name) {
    alert('Please enter a display name for the new god.');
    return;
  }

  const displayName = document.getElementById('godEditName')?.value?.trim() || name;
  const domain = document.getElementById('godEditDomain')?.value?.trim() || '';
  const provider = document.getElementById('godEditProvider')?.value || '';
  const model = document.getElementById('godEditModel')?.value || '';
  const color = document.getElementById('godEditColor')?.value?.trim() || '#748FFC';
  const soul = document.getElementById('godEditSoul')?.value || '';

  try {
    // Step 1: Create the profile if it's new
    if (isNew) {
      const createResp = await fetch('/api/profile/create', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ name })
      });
      if (!createResp.ok) {
        const err = await createResp.json().catch(() => ({error: 'Creation failed'}));
        throw new Error(err.error || 'Failed to create god');
      }
      _editingGodName = name;
      if ($('godEditorTitle')) $('godEditorTitle').innerText = `Edit: ${name}`;
    }

    // Step 2: Update metadata
    const metaResp = await fetch('/api/gods/' + encodeURIComponent(name) + '/metadata', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ display_name: displayName, domain, color })
    });
    if (!metaResp.ok) throw new Error('Failed to save metadata');

    // Step 3: Upload icon if changed
    if (_pendingGodIconData) {
      const iconResp = await fetch('/api/gods/' + encodeURIComponent(name) + '/icon', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ image: _pendingGodIconData })
      });
      if (!iconResp.ok) throw new Error('Failed to upload icon');
      const iconData = await iconResp.json();
      _pendingGodIconData = null;

      // Step 3b: Update metadata with icon URL so it persists on reload
      if (iconData && iconData.url) {
        await fetch('/api/gods/' + encodeURIComponent(name) + '/metadata', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
    ...
    // Step 4: Save SOUL.md if non-empty
    if (soul) {
      const soulResp = await fetch('/api/gods/' + encodeURIComponent(name) + '/soul', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
      ...
      if (!soulResp.ok) throw new Error('Failed to save SOUL.md');
    }

    // Step 5: Save model selection
    if (model) {
      const modelResp = await fetch('/api/gods/' + encodeURIComponent(name) + '/model', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
    ...
    // Success
    showToast('God saved successfully');
    await loadGodList();
    if (isNew) {
      openGodEditor(name);
    }
...
async function deleteGodEditor() {
  if (!_editingGodName) return;
  if (!confirm(`Delete god "${_editingGodName}"? This cannot be undone.`)) return;
  try {
    const resp = await fetch('/api/profile/delete', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ name: _editingGodName })
    });
    if (!resp.ok) throw new Error('Delete failed');
    showToast('God deleted');
    _editingGodName = null;
    $('btnSaveGodEditor').style.display = 'none';
    $('btnDeleteGodEditor').style.display = 'none';
    const body = $('godEditorBody');
    if (body) body.style.display = 'none';
    const empty = $('godEditorEmpty');
    if (empty) empty.style.display = 'flex';
    const title = $('godEditorTitle');
    if (title) title.innerText = 'God Editor';
    await loadGodList();
  } catch (e) {
    showToast('Error: ' + e.message);
  }
...
// ═════════════════════════════════════════════════════════════════════════
// Forge Popup — Hephaestus SOUL.md creation interview
// ═════════════════════════════════════════════════════════════════════════

let _forgeGodName = '';
let _forgeSoulDraft = null;
let _forgeActive = false;

function openForgePopup(godName) {
  _forgeGodName = godName;
  _forgeSoulDraft = null;
  _forgeActive = false;
  const ov = document.getElementById('forgeOverlay');
  ov.dataset.forgeHidden = 'false';
  const title = document.getElementById('forgeGodName');
  if (title) title.textContent = godName;
  _forgeResetUI(true);
  _forgeSendAction('start', '');
}

function closeForgePopup() {
  document.getElementById('forgeOverlay').dataset.forgeHidden = 'true';
  _forgeGodName = '';
  _forgeSoulDraft = null;
}

function _forgeResetUI(loading) {
  const msgsEl = document.getElementById('forgeMessages');
  if (msgsEl) msgsEl.innerHTML = '';
  const typing = document.getElementById('forgeTyping');
  if (typing) typing.dataset.forgeHidden = 'true';
  const input = document.getElementById('forgeInput');
  const sendBtn = document.getElementById('forgeSendBtn');
  if (input) { input.value = ''; input.disabled = loading; }
  if (sendBtn) { sendBtn.disabled = loading; sendBtn.textContent = loading ? '...' : 'Send'; }
  const actions = document.getElementById('forgeActions');
  if (actions) actions.dataset.forgeHidden = 'true';
  const errorBar = document.getElementById('forgeError');
  if (errorBar) errorBar.dataset.forgeHidden = 'true';
  if (loading) {
    const t = document.getElementById('forgeTyping');
    if (t) t.dataset.forgeHidden = 'false';
  }
}

function _addForgeMsg(role, content) {
  const msgsEl = document.getElementById('forgeMessages');
  if (!msgsEl) return;
  const div = document.createElement('div');
  ...
  avatar.textContent = role === 'hephaestus' ? '\u2692\uFE0F' : '\uD83D\uDC64';
  ...
  let html = content
    ...
    .replace(/```markdown\n?([\s\S]*?)```/g, '<pre class="forge-code-block"><code>$1</code></pre>')
...
function _forgeShowError(msg) {
  const bar = document.getElementById('forgeError');
  if (!bar) return;
  bar.textContent = '\u26A0\uFE0F ' + msg;
  bar.dataset.forgeHidden = 'false';
}

function _forgeShowActions(show) {
  const a = document.getElementById('forgeActions');
  if (a) a.dataset.forgeHidden = show ? 'false' : 'true';
}

async function _forgeSendAction(action, message) {
  if (_forgeActive) return;
  if (!_forgeGodName) {
    _forgeShowError('Please save the god first before forging its soul.');
    const input = document.getElementById('forgeInput');
    const sendBtn = document.getElementById('forgeSendBtn');
    const typing = document.getElementById('forgeTyping');
    if (typing) typing.dataset.forgeHidden = 'true';
  ...
  _forgeActive = true;
  const typing = document.getElementById('forgeTyping');
  if (typing && action !== 'accept') typing.dataset.forgeHidden = 'false';
  const input = document.getElementById('forgeInput');
  const sendBtn = document.getElementById('forgeSendBtn');
  ...
  try {
    ...
    if (action === 'chat') payload.message = message;
    if (action === 'accept') payload.soul_draft = _forgeSoulDraft;
    const resp = await fetch('/api/gods/' + encodeURIComponent(_forgeGodName) + '/forge', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!resp.ok || data.error) {
      _forgeShowError(data.error || 'Request failed');
      _forgeActive = false;
      if (input) { input.disabled = false; input.value = ''; }
      if (sendBtn) { sendBtn.disabled = false; sendBtn.textContent = 'Send'; }
      return;
    ...
    if (action === 'accept') {
      _addForgeMsg('system', '\u2705 SOUL.md saved! The god has been forged.');
      _forgeSoulDraft = null;
      _forgeShowActions(false);
      if (input) input.disabled = true;
      if (sendBtn) sendBtn.textContent = 'Saved!';
      setTimeout(() => {
        document.getElementById('forgeOverlay').dataset.forgeHidden = 'true';
        if (typeof _editingGodName !== 'undefined' && _editingGodName === _forgeGodName) {
          openGodEditor(_forgeGodName);
        }
      }, 2000);
      _forgeActive = false;
      return;
    }
    _forgeSoulDraft = data.soul_draft || null;
    if (action === 'chat') _addForgeMsg('user', message);
    _addForgeMsg('hephaestus', data.reply || '(no response)');
    _forgeShowActions(!!_forgeSoulDraft);
    if (input) { input.disabled = false; input.value = ''; }
    if (sendBtn) { sendBtn.disabled = false; sendBtn.textContent = 'Send'; }
    if (input) input.focus();
  } catch (e) {
    const t = document.getElementById('forgeTyping');
    if (t) t.dataset.forgeHidden = 'true';
    _forgeShowError('Network error: ' + e.message);
    const input = document.getElementById('forgeInput');
    const sendBtn = document.getElementById('forgeSendBtn');
    if (input) { input.disabled = false; input.value = ''; }
    if (sendBtn) { sendBtn.disabled = false; sendBtn.textContent = 'Send'; }
...
function sendForgeMessage() {
  ...
  if (!msg) return;
  input.value = '';
  _forgeSendAction('chat', msg);
}

function acceptForgeSoul() {
  if (_forgeActive || !_forgeSoulDraft) return;
  _forgeSendAction('accept', '');
}

function refineForgeSoul() {
  _forgeShowActions(false);
  const input = document.getElementById('forgeInput');
  const sendBtn = document.getElementById('forgeSendBtn');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (forgeInput) {
    forgeInput.addEventListener('keydown', function(e) {
      if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendForgeMessage(); }
    });
  }
  const acceptBtn = document.getElementById('forgeAcceptBtn');
  if (acceptBtn) acceptBtn.addEventListener('click', acceptForgeSoul);
  const refineBtn = document.getElementById('forgeRefineBtn');
  if (refineBtn) refineBtn.addEventListener('click', refineForgeSoul);
  const sendBtn = document.getElementById('forgeSendBtn');
  if (sendBtn) sendBtn.addEventListener('click', sendForgeMessage);
});
