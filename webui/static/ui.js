const S={session:null,messages:[],entries:[],busy:false,pendingFiles:[],toolCalls:[],activeStreamId:null,currentDir:'.',activeProfile:'default',showHiddenWorkspaceFiles:false}; window.S=S;
let _godMap={}; window._godMap=_godMap;  // populated from /api/gods — name -> {icon, color, display_name, ...}
...
function _stripWorkspaceDisplayPrefix(text){
  return String(text||'').replace(/^\s*\[(Workspace|Forge):[^\]]+\]\s*/,'').trim();
}
...
const _MOBILE_CONFIG_BASE_LABEL='Forge, model, reasoning, and context settings';
...
function _finishAppDialog(result, restoreFocus=true){
  ...
  // Restore chat position when dialog closes
  _adjustChatForDrawer(false);
}

/** When the clarify/dialog drawer opens, push the chat content up so
 *  the assistant's response remains visible above the drawer. */
function _adjustChatForDrawer(open){
  const msg=$('messages');
  if(!msg) return;
  if(open){
    const dialog=$('appDialog');
    const h=dialog ? Math.min(dialog.offsetHeight, window.innerHeight*0.48) : window.innerHeight*0.48;
    msg.style.paddingBottom=Math.round(h+16)+'px';
    requestAnimationFrame(()=>{
      if(typeof scrollToBottom==='function') scrollToBottom();
      setTimeout(()=>{ if(typeof scrollToBottom==='function') scrollToBottom(); }, 150);
    });
  } else {
    msg.style.paddingBottom='';
  }
}
...
function showConfirmDialog(opts={}){
  ...
  // Shift the chat up so messages are visible above the drawer
  _adjustChatForDrawer(true);
...
async function refreshTelegram() {
  // Refresh the session sidebar list AND the active session messages
  try {
    await Promise.all([
      typeof renderSessionList === 'function' ? renderSessionList() : Promise.resolve(),
      S.session ? refreshSession() : Promise.resolve(),
    ]);
...
function syncTopbar(){
  if(!S.session){
    document.title=(typeof S !== 'undefined' && S.activeProfile && S.activeProfile !== 'default')
      ? S.activeProfile.charAt(0).toUpperCase() + S.activeProfile.slice(1)
    ...
    if(typeof _syncHermesPanelSessionActions==='function') _syncHermesPanelSessionActions();
      ...
      if(sidebarName && sidebarName.textContent==='Forge'){
  ...
  const _profileDisplayName = (typeof S !== 'undefined' && S.activeProfile && S.activeProfile !== 'default')
    ? S.activeProfile.charAt(0).toUpperCase() + S.activeProfile.slice(1)
    : (window._botName||'Hermes');
  document.title=sessionTitle+' \u2014 '+_profileDisplayName;
...
function _assistantRoleHtml(tsTitle='', tpsText=''){
  // Use the active profile name when a non-default profile is selected,
  // so message bubbles show the right god/agent name (Apollo, Hephaestus, etc.)
  // instead of always showing the global bot name ('Hermes').
  const _profileName = (typeof S !== 'undefined') ? (S.activeProfile || 'default') : 'default';
  const _god = _godMap[_profileName];
  let _displayName, _iconHtml;
  if(_god){
    _displayName = _god.display_name;
    const iconVal = _god.icon || '';
    if (typeof iconVal === 'string' && (iconVal.startsWith('http') || iconVal.startsWith('/api/'))) {
      _iconHtml = '<img src="' + iconVal.replace(/"/g,'&quot;') + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block">';
    } else {
      _iconHtml = esc(iconVal || _displayName.charAt(0).toUpperCase());
    }
  } else {
    // Fallback before first /api/gods fetch
    _displayName = (typeof S !== 'undefined' && S.activeProfile && S.activeProfile !== 'default')
      ? S.activeProfile.charAt(0).toUpperCase() + S.activeProfile.slice(1)
      : (window._botName || 'Hermes');
    _iconHtml = esc(_displayName.charAt(0).toUpperCase());
  }
  const tps=(isTpsDisplayEnabled()&&tpsText)?`<span class="msg-tps-inline" title="Tokens per second">${esc(tpsText)}</span>`:'';
  return `<div class="msg-role assistant" ${tsTitle?`title="${esc(tsTitle)}"`:''}><div class="role-icon assistant">${_iconHtml}</div><span style="font-size:12px">${esc(_displayName)}</span>${tps}</div>`;
}
...
function renderMessages(options){
  ...
  for(let vi=0;vi<renderVisWithIdx.length;vi++){
    ...
    const boonBtn  = !isUser ? `<button class="msg-action-btn msg-boon-btn" title="Promote to Boon" onclick="promoteMsgToBoon(this)">📜</button>` : '';
    ...
    const footHtml = `<div class="msg-foot">${timeHtml}<span class="msg-actions">${editBtn}${ttsBtn}${boonBtn}${forkBtn}${copyBtn}${retryBtn}</span></div>`;
...
// ── God Detail Panel (Phase C) ────────────────────────────────────────────

function renderGodDetail(god) {
  closeGodDetail();
  const container = document.getElementById('sidebarGodRoster');
  if (!container || !god) return;
  const isDefault = god.is_default;
  const isAwake = god.gateway_state === 'awake';
  const stateLabel = isAwake ? '🟢 Awake' : '💤 Sleeping';
  const stateClass = isAwake ? 'awake' : 'sleeping';

  const detail = document.createElement('div');
  detail.className = 'god-detail-card';
  detail.id = 'godDetailCard';
  // Precompute icon HTML: <img> for URL icons, text for emoji
  var detailIcon = god.icon || '🧑‍💻';
  var iconHtml_ = (typeof detailIcon === 'string' && (detailIcon.startsWith('http') || detailIcon.startsWith('/api/')))
    ? '<img src="' + detailIcon.replace(/"/g,'&quot;') + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block">'
    : detailIcon;
  detail.innerHTML = `
    <button class="god-detail-close" onclick="closeGodDetail()">&times;</button>
    <div class="god-detail-header">
      <span class="god-detail-icon" style="background:${god.color}44;overflow:hidden">${iconHtml_}</span>
      <span class="god-detail-name">${god.display_name}</span>
      <span class="god-detail-domain">${god.domain || 'Specialist'}</span>
    </div>
    <div class="god-detail-body">
      <div class="god-detail-row"><label>Name</label><span>${god.name}</span></div>
      <div class="god-detail-row"><label>Color</label><span class="god-detail-color-swatch" style="background:${god.color}">${god.color}</span></div>
      <div class="god-detail-row"><label>Status</label><span class="god-detail-state ${stateClass}">${stateLabel}</span></div>
      <div class="god-detail-row"><label>Model</label><span>${god.model || 'Default'}</span></div>
      <div class="god-detail-row"><label>Provider</label><span>${god.provider || 'Default'}</span></div>
      <div class="god-detail-row"><label>Default</label><span>${isDefault ? 'Yes' : 'No'}</span></div>
    </div>
    <div class="god-detail-actions">
      ${!isAwake && !isDefault ? `<button class="god-invoke-btn" onclick="invokeGod('${god.name}', this)">⚡ Invoke</button>` : ''}
      ${isAwake && !isDefault ? `<button class="god-sleep-btn" onclick="sleepGod('${god.name}', this)">💤 Sleep</button>` : ''}
    </div>
  `;
...
function closeGodDetail() {
  const existing = document.getElementById('godDetailCard');
  if (existing) existing.remove();
}

// ── Invoke / Sleep Actions (Phase C) ─────────────────────────────────────

async function invokeGod(name, btn) {
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Invoking...'; }
  try {
    const res = await fetch(`/api/gods/${name}/invoke`, { method: 'POST' });
    const data = await res.json();
    if (data.ok) {
      showToast(`${name.charAt(0).toUpperCase() + name.slice(1)} invoked`);
      // Auto-refresh state after a brief delay so the process has time to start
      setTimeout(refreshGodStates, 1500);
    } else {
      showToast(data.error || `Failed to invoke ${name}`, 'error');
    }
  } catch (e) {
    showToast(`Failed to invoke ${name}: ${e.message}`, 'error');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '⚡ Invoke'; }
  }
}

async function sleepGod(name, btn) {
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Putting to sleep...'; }
  try {
    const res = await fetch(`/api/gods/${name}/sleep`, { method: 'POST' });
    const data = await res.json();
    if (data.ok) {
      showToast(`${name.charAt(0).toUpperCase() + name.slice(1)} put to sleep`);
      setTimeout(refreshGodStates, 1000);
    } else {
      showToast(data.error || `Failed to sleep ${name}`, 'error');
    }
  } catch (e) {
    showToast(`Failed to sleep ${name}: ${e.message}`, 'error');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '💤 Sleep'; }
  }
}

// ── Export Modal ────────────────────────────────────────────────────────

async function openExportModal(godName) {
  closeExportModal();
  ...
  modal.innerHTML = `
    <div class="export-modal-header">
      <span>📦 Export <span id="exportGodName">${godName}</span></span>
      <button class="export-modal-close" onclick="closeExportModal()">&times;</button>
    </div>
    <div class="export-modal-body" id="exportModalBody">
      <div class="export-loading">Loading...</div>
    </div>
  `;
  overlay.appendChild(modal);
  document.body.appendChild(overlay);

  // Fetch codex folders and build the form
  try {
    const res = await fetch(`/api/gods/${godName}/codex-folders`);
    const data = await res.json();
    const body = document.getElementById('exportModalBody');
    let html = `
      <label class="export-check-row">
        <input type="checkbox" id="exportIncludeSkills" checked>
        <span>Include skills</span>
      </label>
    `;
    if (data.exists && data.folders.length > 0) {
      html += `<div class="export-section-label">Codex folders (select which to include)</div>
               <div class="export-codex-tree">`;
      data.folders.forEach(f => {
        html += `<label class="export-check-row export-codex-folder">
          <input type="checkbox" class="export-codex-cb" value="${f}" checked>
          <span>📁 ${f}</span>
        </label>`;
      });
      html += `<div class="export-codex-actions">
        <button class="export-codex-btn" onclick="document.querySelectorAll('.export-codex-cb').forEach(c=>c.checked=true)">All</button>
        <button class="export-codex-btn" onclick="document.querySelectorAll('.export-codex-cb').forEach(c=>c.checked=false)">None</button>
      </div></div>`;
    } else if (data.exists) {
      html += `<div class="export-no-codex">No codex folders available (auto-excluded: archive, sessions, distilled).</div>`;
    } else {
      html += `<div class="export-no-codex">No codex found for this god.</div>`;
    }
    html += `
      <div class="export-actions">
        <button class="god-export-btn" onclick="triggerExport('${godName}')" style="flex:1">📦 Export Now</button>
        <button class="god-sleep-btn" onclick="closeExportModal()" style="flex:0.5">Cancel</button>
      </div>
    `;
    body.innerHTML = html;
  } catch (e) {
    document.getElementById('exportModalBody').innerHTML = `<div class="export-error">Failed to load: ${e.message}</div>`;
  }
}
...
async function triggerExport(godName) {
  const includeSkills = document.getElementById('exportIncludeSkills').checked;
  const cb = document.querySelectorAll('.export-codex-cb:checked');
  const codexFolders = Array.from(cb).map(c => c.value);

  const btn = document.querySelector('.export-actions .god-export-btn');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Bundling...'; }

  try {
    const res = await fetch(`/api/gods/${godName}/export`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        include_skills: includeSkills,
        codex_folders: codexFolders.length > 0 ? codexFolders : null
      })
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      showToast(err.error || `Export failed (${res.status})`, 3000, 'error');
      return;
    }

    // Read blob but DON'T auto-download yet — show prompt instead
    const blob = await res.blob();
    const disposition = res.headers.get('Content-Disposition') || '';
    const match = disposition.match(/filename="?([^";\n]+)"?/);
    const filename = match ? match[1] : `${godName}-export.tar.gz`;
    const sizeKB = Math.round(blob.size / 1024);

    // Store blob for later download
    const blobUrl = URL.createObjectURL(blob);

    // Show success view in modal with Download + Submit options
    const body = document.getElementById('exportModalBody');
    body.innerHTML = `
      <div style="text-align:center;padding:12px 0">
        <div style="font-size:40px;margin-bottom:8px">✅</div>
        <div style="font-size:15px;font-weight:700;color:var(--text,#e2e8f0)">Bundle exported!</div>
        <div style="font-size:12px;color:var(--muted,#94a3b8);margin-top:4px">${filename} (${sizeKB} KB)</div>
      </div>
      <div class="export-actions" style="flex-direction:column;gap:8px;margin-top:8px">
        <button class="god-export-btn" onclick="downloadExportBlob('${godName}')" style="width:100%">📥 Download Bundle</button>
        <button class="summon-btn" onclick="submitToSummons('${godName}')" id="submitSummonBtn" style="width:100%">🌟 Submit to Pantheon-Summons</button>
        <button class="god-sleep-btn" onclick="closeExportModal()" style="width:100%">Close</button>
      </div>
    `;
    // Store the blob data on window for the download function
    window._exportBlobs = window._exportBlobs || {};
    window._exportBlobs[godName] = { blob: blob, url: blobUrl, filename: filename };
  } catch (e) {
    showToast(`Export failed: ${e.message}`, 3000, 'error');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '📦 Export Now'; }
  }
}

function downloadExportBlob(godName) {
  const data = window._exportBlobs && window._exportBlobs[godName];
  if (!data) return showToast('Export data not found', 2000, 'error');
...
async function submitToSummons(godName) {
  const btn = document.getElementById('submitSummonBtn');
  if (!btn) return;
  btn.disabled = true;
  btn.innerHTML = '⏳ Submitting... <span class="spinner">⟳</span>';

  // Read the same codex/skills selections from the export modal
  const includeSkills = document.getElementById('exportIncludeSkills')?.checked ?? true;
  const cb = document.querySelectorAll('.export-codex-cb:checked');
  const codexFolders = Array.from(cb).map(c => c.value);

  try {
    const res = await fetch(`/api/gods/${godName}/submit-to-summons`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        include_skills: includeSkills,
    ...
    if (data.error) {
      btn.innerHTML = `⚠️ ${data.error}`;
      btn.disabled = false;
      setTimeout(() => { btn.innerHTML = '🌟 Submit to Pantheon-Summons'; }, 3000);
    } else {
      btn.innerHTML = '✅ PR Created!';
      if (data.pr_url) {
        // Show the PR link
        const body = document.getElementById('exportModalBody');
        body.innerHTML += `
          <div style="text-align:center;padding:12px;margin-top:8px;background:var(--card-bg,#1e1e36);border-radius:8px;font-size:13px">
            🌟 <a href="${_escH(data.pr_url)}" target="_blank" style="color:#22d3ee">View PR on GitHub</a>
          </div>
        `;
      }
    }
...
// ── State polling (Phase C) ──────────────────────────────────────────────

let _godPollTimer = null;
const GOD_POLL_INTERVAL = 30000;  // 30 seconds

async function refreshGodStates() {
  try {
    ...
    // Update state dots on existing rail buttons without full re-render
    data.gods.forEach(god => {
      // Rail dot
      const railBtn = document.querySelector(`.god-rail-btn[aria-label="${god.display_name}"]`);
      if (railBtn) {
        const dot = railBtn.querySelector('.god-state-dot');
        if (dot) dot.className = 'god-state-dot ' + (god.gateway_state || 'sleeping');
        if (god.is_active) {
          railBtn.classList.add('active');
          railBtn.style.borderColor = god.color;
        } else {
          railBtn.classList.remove('active');
          railBtn.style.borderColor = 'transparent';
        }
      }
      // Sidebar row state dot
      const rows = document.querySelectorAll('.sidebar-god-row');
      rows.forEach(row => {
        const nameEl = row.querySelector('.god-row-name');
        if (nameEl && nameEl.textContent === god.display_name) {
          const dot = row.querySelector('.row-state-dot');
          if (dot) dot.className = 'row-state-dot ' + (god.gateway_state || 'sleeping');
          row.classList.toggle('active', !!god.is_active);
        }
      });
    });

    // Update the detail card if visible
    const detailCard = document.getElementById('godDetailCard');
    if (detailCard) {
      const activeGod = data.gods.find(g => g.is_active);
      if (activeGod) {
        // Re-render detail card to reflect new state
        renderGodDetail(activeGod);
      }
    }

    // ── God-specific accent theming ──
  } catch (e) {
    // Silently fail — polling is best-effort
  }
}

function startGodPolling() {
  stopGodPolling();
  _godPollTimer = setInterval(refreshGodStates, GOD_POLL_INTERVAL);
}

function stopGodPolling() {
  if (_godPollTimer) {
    clearInterval(_godPollTimer);
    _godPollTimer = null;
  }
}
...
async function loadGodRoster() {
  const container = document.getElementById('godRail');
  if (!container) return;
  try {
    const res = await fetch('/api/gods');
    if (!res.ok) return;
    const data = await res.json();
    if (!data.gods || !data.gods.length) return;
    // Cache god metadata for chat icon display
    _godMap = {}; window._godMap = _godMap;
    data.gods.forEach(g => { _godMap[g.name] = g; });
    // ── Rail roster (icon-only) ──
    container.innerHTML = '';
    // ── Sidebar roster (full info) ──
    const sidebarContainer = document.getElementById('sidebarGodRoster');
    if (sidebarContainer) {
    ...
    data.gods.forEach(god => {
      // Rail button
      const btn = document.createElement('button');
      btn.className = 'god-rail-btn has-tooltip' + (god.is_active ? ' active' : '');
      btn.dataset.tooltip = god.display_name;
      btn.setAttribute('aria-label', god.display_name);
      btn.style.borderColor = god.is_active ? god.color : 'transparent';
      btn.style.setProperty('--accent', god.color);
      // Render icon: <img> for URLs, textContent for emoji
      var iconVal = god.icon || '🧑‍💻';
      if (typeof iconVal === 'string' && (iconVal.startsWith('http') || iconVal.startsWith('/api/'))) {
        btn.innerHTML = '<img src="' + iconVal.replace(/"/g,'&quot;') + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block">';
      } else {
        btn.textContent = iconVal;
      }
      btn.onclick = () => switchGod(god.name);
      const dhState = god.gateway_state || 'sleeping';
      const dot = document.createElement('span');
      dot.className = 'god-state-dot ' + dhState;
      dot.style.borderColor = 'var(--sidebar)';
      btn.appendChild(dot);
      ...
      if (sidebarContainer) {
        ...
        row.className = 'sidebar-god-row' + (god.is_active ? ' active' : '');
        row.onclick = () => {
          switchGod(god.name);
          // Show detail card
          renderGodDetail(god);
        ...
        iconWrap.className = 'god-row-icon';
        iconWrap.style.background = god.color + '22';
        // Render icon: <img> for URLs, textContent for emoji
        var rowIconVal = god.icon || '🧑‍💻';
        if (typeof rowIconVal === 'string' && (rowIconVal.startsWith('http') || rowIconVal.startsWith('/api/'))) {
          iconWrap.innerHTML = '<img src="' + rowIconVal.replace(/"/g,'&quot;') + '" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block">';
        } else {
          iconWrap.textContent = rowIconVal;
        }
        const rowDot = document.createElement('span');
        rowDot.className = 'row-state-dot ' + (god.gateway_state || 'sleeping');
        iconWrap.appendChild(rowDot);
        row.appendChild(iconWrap);
        // Info
        const infoWrap = document.createElement('span');
        infoWrap.className = 'god-row-info';
        const nameSpan = document.createElement('span');
        nameSpan.className = 'god-row-name';
        nameSpan.textContent = god.display_name;
        const domainSpan = document.createElement('span');
        domainSpan.className = 'god-row-domain';
        domainSpan.textContent = god.domain || 'Specialist';
        infoWrap.appendChild(nameSpan);
        infoWrap.appendChild(domainSpan);
    ...
    if (activeGod) {
      ...
      // Update active rail button glow
      document.querySelectorAll('.god-rail-btn').forEach(b => b.classList.remove('active'));
      const activeBtn = document.querySelector(`.god-rail-btn[aria-label="${activeGod.display_name}"]`);
    ...
    // Sync glow with roster state
    _updateGodGlow();

  } catch (e) {
    console.warn('[pantheon] Failed to load god roster:', e);
  }
}

/* ── God Glow ── */
function _updateGodGlow() {
  // Use inline <style> tag — bypasses SW cache and CSS variable inheritance
  let styleEl = document.getElementById('god-glow-style');
  if (!styleEl) {
    styleEl = document.createElement('style');
    styleEl.id = 'god-glow-style';
    document.head.appendChild(styleEl);
  }
  const profileName = (typeof S !== 'undefined') ? (S.activeProfile || 'default') : 'default';
  const god = window._godMap ? _godMap[profileName] : null;
  if (god && god.color) {
    const c = god.color;
    ...
    styleEl.textContent = '';
    console.log('[pantheon] God glow OFF - no god/color for', profileName, god ? 'no color' : 'no god');
  }
}

async function switchGod(godName) {
  try {
    // Use the full SillyTavern profile switching pipeline
    if (typeof switchToProfile === 'function') {
      await switchToProfile(godName);
    } else {
      ...
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      if (typeof S !== 'undefined') S.activeProfile = data.active || godName;
      if (typeof applyBotName === 'function') applyBotName();
      if (typeof syncTopbar === 'function') syncTopbar();
    }
    // Refresh the god roster and session list
    loadGodRoster();
    _updateGodGlow();
    if (typeof loadSessions === 'function') loadSessions();
  } catch (e) {
    showToast(`Failed to invoke ${godName}: ${e.message}`, 'error');
  }
}
...
function toggleIdeasDropdown() {
  const existing = document.getElementById('ideasDropdown');
  if (existing) { existing.remove(); return; }
  const btn = document.getElementById('btnIdeas');
  if (!btn) return;
  const dd = document.createElement('div');
  dd.id = 'ideasDropdown';
  dd.className = 'titlebar-dropdown ideas-dropdown';
  dd.innerHTML =
    '<div class="titlebar-dropdown-header">Ideas</div>' +
    '<div class="titlebar-dropdown-body"><div style="padding:12px;color:var(--muted);font-size:12px">Loading...</div></div>' +
    '<div class="ideas-footer">' +
    '<button class="ideas-add-btn" onclick="toggleAddIdeaForm()" type="button">＋ Add Idea</button>' +
    '<div class="ideas-add-form" style="display:none">' +
    '<div class="ideas-form-row"><select id="ideasAddSection" class="ideas-form-select"></select></div>' +
    '<div class="ideas-form-row"><input id="ideasAddName" class="ideas-form-input" placeholder="Idea name..." /></div>' +
    '<div class="ideas-form-row"><textarea id="ideasAddNotes" class="ideas-form-textarea" placeholder="Notes (optional)" rows="2"></textarea></div>' +
    '<div class="ideas-form-actions">' +
    '<button class="ideas-form-cancel" onclick="toggleAddIdeaForm()" type="button">Cancel</button>' +
    '<button class="ideas-form-submit" onclick="submitNewIdea()" type="button">Add</button>' +
    '</div></div></div>';
  document.body.appendChild(dd);
  const rect = btn.getBoundingClientRect();
  dd.style.top = (rect.bottom + 4) + 'px';
  // Anchor to right edge but prevent overflow on either side.
  // Three strategies tried in order:
  //   1. Right-anchor (dropdown right edge aligns with button right edge)
  //   2. Left-anchor  (dropdown left edge aligns with button left edge)
  //   3. Center-fill  (when neither anchor fits the viewport)
  var ddW = Math.min(420, window.innerWidth - 16);
  var rEdge = window.innerWidth - rect.right;
  var leftSafe = Math.max(8, rect.left);
  var rightSafe = window.innerWidth - 8;
  var maxLeft = rightSafe - ddW;
  if (rect.right >= ddW) {
    // Enough room to right-anchor — dropdown extends left from rect.right
    dd.style.right = rEdge + 'px';
    dd.style.left = 'auto';
  } else if (rect.left + ddW <= rightSafe) {
    // Right-anchor would overflow left, but left-anchor fits
    dd.style.right = 'auto';
    dd.style.left = Math.max(8, rect.left) + 'px';
  } else {
    // Neither anchor fits — center within viewport with 8px padding.
    // Override the CSS max-width so it actually spans the full screen
    // (CSS has .ideas-dropdown { max-width: min(90vw, 480px) } which
    // clamps smaller than viewport on mobile).
    dd.style.right = '8px';
    dd.style.left = '8px';
  ...
  fetchAndRenderIdeas(dd);

  setTimeout(function() {
    document.addEventListener('click', _closeTitlebarDropdown, {once:true});
    setTimeout(function() {
      document.addEventListener('click', _closeTitlebarDropdown, {once:true});
    }, 100);
  }, 0);
...
function fetchAndRenderIdeas(dd) {
  fetch('/api/ideas')
    .then(r => r.json())
    .then(data => {
      var body = dd.querySelector('.titlebar-dropdown-body');
      if (!body) return;
      renderIdeasBody(body, data);
      // Populate section dropdown in add form
      var sel = document.getElementById('ideasAddSection');
      if (sel && data.sections) {
        sel.innerHTML = '';
    ...
    .catch(function() {
      var body = dd.querySelector('.titlebar-dropdown-body');
      if (body) body.innerHTML = '<div style="padding:12px;color:var(--error,#e05);font-size:12px">Failed to load ideas.</div>';
    });
}

function renderIdeasBody(body, data) {
  if (data.error || !data.sections || !data.sections.length) {
    body.innerHTML = '<div style="padding:16px;color:var(--muted);font-size:13px;text-align:center">No ideas yet. Tell me about something you want to build!</div>';
    return;
  }
  var html = '';
  ...
  data.sections.forEach(function(section) {
    ...
    html += '<div class="ideas-section" data-section="' + _esc(section.name) + '">';
    html += '<div class="ideas-section-name">' + _esc(section.name) + ' <span style="font-weight:400;opacity:.6">(' + section.entries.length + ')</span></div>';
    section.entries.forEach(function(entry) {
      var statusMatch = entry.lines.join(' ').match(/\*\*Status:\*\*\s*([\w-]+)/i);
      var status = statusMatch ? statusMatch[1].toLowerCase() : 'idea';
      var statusClass = status.replace(/[^a-z]/g, '');
      var notesMatch = entry.lines.join(' ').match(/\*\*Notes:\*\*\s*(.*?)(?=\s*\n\s*\n|$)/i);
      var notes = notesMatch ? notesMatch[1].trim() : '';
      var addedMatch = entry.lines.join(' ').match(/\*\*Added:\*\*\s*([\d-]+)/i);
      var added = addedMatch ? addedMatch[1].trim() : '';
      var notesPreview = notes.length > 80 ? notes.substring(0, 80) + '\u2026' : notes;
      html += '<div class="ideas-entry" draggable="true" data-entry="' + _esc(entry.name) + '" data-status="' + _esc(status) + '" data-notes="' + _esc(notes) + '">';
      html += '<span class="ideas-drag-handle" title="Drag to reorder">\u283f</span>';
      html += '<div class="ideas-entry-body">';
      html += '<div class="ideas-entry-top">';
      html += '<span class="ideas-status ideas-status-' + statusClass + '">' + _esc(status) + '</span>';
      html += '<span class="ideas-name">' + _esc(entry.name) + '</span>';
      html += '</div>';
      if (notesPreview) {
        html += '<div class="ideas-notes-preview">' + _esc(notesPreview) + '</div>';
      }
      if (added) {
        html += '<div class="ideas-meta">Added ' + _esc(added) + '</div>';
      }
      html += '</div>';
      html += '<span class="ideas-delete-btn" title="Delete this idea">\u2715</span>';
      html += '</div>';
    });
    html += '</div>';
  });
  ...
  makeIdeasSortable(body);

  // Click handlers for each entry
  body.querySelectorAll('.ideas-entry').forEach(function(row) {
    var statusEl = row.querySelector('.ideas-status');
    var sectionEl = row.closest('.ideas-section');
    var sectionName = sectionEl ? (sectionEl.getAttribute('data-section') || sectionEl.querySelector('.ideas-section-name').textContent.replace(/\s*\(.*?\)\s*$/, '')) : '';
    var entryName = row.getAttribute('data-entry');
    if (!entryName) entryName = row.querySelector('.ideas-name').textContent;

    // Status badge - cycle through statuses
    if (statusEl) {
      statusEl.style.cursor = 'pointer';
      statusEl.title = 'Click to cycle status: idea \u2192 planning \u2192 in-progress \u2192 shelved \u2192 done';
      statusEl.addEventListener('click', function(e) {
        e.stopPropagation();
        var currentStatus = statusEl.textContent.trim();
        var statuses = ['idea', 'planning', 'in-progress', 'shelved', 'done'];
        ...
        .then(function(resp) {
          ...
          statusEl.className = 'ideas-status ideas-status-' + nextStatus.replace(/[^a-z]/g, '');
        })
        .catch(function(err) { alert('Status update error: ' + (err && err.message ? err.message : 'Network error')); });
      });
    }

    // Delete button
    var delBtn = row.querySelector('.ideas-delete-btn');
    if (delBtn) {
      delBtn.addEventListener('click', function(e) {
        e.stopPropagation();
        var confirmMsg = 'Delete "' + entryName + '"?';
        ...
        .then(function(resp) {
          ...
          renderIdeasBody(body, resp);
        })
        .catch(function(err) { alert('Delete error: ' + (err && err.message ? err.message : 'Network error')); });
      });
    }

    // Click name - open edit modal
    var nameEl = row.querySelector('.ideas-name');
    if (nameEl) {
      nameEl.style.cursor = 'pointer';
      nameEl.addEventListener('click', function(e) {
        e.stopPropagation();
        var currentNotes = row.getAttribute('data-notes') || '';
        var currentStatus = row.getAttribute('data-status') || 'idea';
        openIdeaEditor(sectionName, entryName, currentStatus, currentNotes, body);
      });
    }
  });
}

/* - HTML5 Drag-to-Reorder for Ideas - */
function makeIdeasSortable(container) {
  var dragSrcEl = null;

  container.addEventListener('dragstart', function(e) {
    var entry = e.target.closest('.ideas-entry');
    ...
    e.dataTransfer.setData('text/plain', entry.getAttribute('data-entry') || '');
    container.querySelectorAll('.ideas-entry').forEach(function(el) {
  ...
  container.addEventListener('dragover', function(e) {
    ...
    var target = e.target.closest('.ideas-entry');
  ...
  container.addEventListener('drop', function(e) {
    ...
    var section = target.closest('.ideas-section');
    if (!section) {
      container.querySelectorAll('.ideas-entry').forEach(function(el) {
        el.classList.remove('drag-over-top', 'drag-over-bottom');
      });
    ...
    // Don't allow cross-section reorder
    var srcSection = dragSrcEl.closest('.ideas-section');
    ...
    if (insertBefore) {
      section.insertBefore(dragSrcEl, target);
    } else {
      var nextSibling = target.nextElementSibling;
      if (nextSibling && nextSibling.closest('.ideas-section') === section) {
        section.insertBefore(dragSrcEl, nextSibling);
      } else {
        section.appendChild(dragSrcEl);
      }
    }
    ...
    // Persist new order
    persistIdeasOrder(section);
  });
}

function persistIdeasOrder(section) {
  var sectionName = section.getAttribute('data-section') ||
    section.querySelector('.ideas-section-name').textContent.replace(/\s*\(.*?\)\s*$/, '');
  var entries = section.querySelectorAll('.ideas-entry');
  var order = [];
  ...
  .then(function(resp) {
    if (resp.error) {
      console.warn('Reorder failed:', resp.error);
    }
  })
  .catch(function(err) {
...
function toggleAddIdeaForm() {
  var form = document.querySelector('.ideas-add-form');
  var btn = document.querySelector('.ideas-add-btn');
  if (!form || !btn) return;
  var isHidden = form.style.display === 'none' || !form.style.display;
  form.style.display = isHidden ? 'block' : 'none';
  btn.style.display = isHidden ? 'none' : '';
  if (isHidden) {
    document.getElementById('ideasAddName').focus();
  }
}

function submitNewIdea() {
  var sel = document.getElementById('ideasAddSection');
  var nameInput = document.getElementById('ideasAddName');
  var notesInput = document.getElementById('ideasAddNotes');
  if (!sel || !nameInput) return;
  var sectionName = sel.value;
  var entryName = nameInput.value.trim();
  if (!entryName) { nameInput.focus(); return; }
  var notes = (notesInput ? notesInput.value : '').trim();

  var submitBtn = document.querySelector('.ideas-form-submit');
  ...
  .then(function(resp) {
    ...
    toggleAddIdeaForm();
    // Re-render body with updated sections
    var body = document.querySelector('#ideasDropdown .titlebar-dropdown-body');
    if (body && resp.sections) {
      renderIdeasBody(body, resp);
      // Re-populate section dropdown
      var sel2 = document.getElementById('ideasAddSection');
      if (sel2 && resp.sections) {
        sel2.innerHTML = '';
...
// Persistent notification queue backed by localStorage.
// Each notification: { id, type, icon, title, body, timestamp, dismissed }

const NOTIF_STORAGE_KEY = 'pantheon_notifications';
const NOTIF_MAX_ACTIVE = 5;
const NOTIF_MAX_TOTAL = 50;
...
const PantheonNotifications = {
  ...
  getActive() {
    return _notifLoad().filter(function(n) { return !n.dismissed; }).slice(0, NOTIF_MAX_ACTIVE);
  },
  getCount() {
  ...
  add(opts) {
    ...
    if (!icon) {
      switch (type) {
        ...
        case 'briefing': icon = '📊'; break;
        case 'update': icon = '🔄'; break;
        case 'health': icon = '⚡'; break;
        case 'god': icon = '🎭'; break;
        case 'boon': icon = '📜'; break;
    ...
    var all = _notifLoad();
    all.unshift(notif);
    if (all.length > NOTIF_MAX_TOTAL) all.length = NOTIF_MAX_TOTAL;
    _notifSave(all);
    _notifUpdateBadge();

    // Show toast for ~4s
    var toastMsg = icon + ' ' + notif.title;
    if (notif.body) toastMsg += ' — ' + notif.body;
    showToast(toastMsg, NOTIF_TOAST_MS, type === 'error' ? 'error' : 'info');

    // Dispatch a custom event so other parts of UI can react
    window.dispatchEvent(new CustomEvent('pantheon-notification', { detail: notif }));
    return notif;
  },
  dismiss(id) {
    var all = _notifLoad();
  ...
  dismissAllOfType(type) {
    var all = _notifLoad();
    for (var i = 0; i < all.length; i++) {
      if (all[i].type === type) all[i].dismissed = true;
    }
    _notifSave(all);
  }
};
...
function toggleNotificationsDropdown() {
  ...
  const active = PantheonNotifications.getActive();
  var headerHtml = '<div class="titlebar-dropdown-header">🔔 Notifications';
  if (active.length > 0) {
    headerHtml += ' <button class="notif-clear-all" onclick="PantheonNotifications.clearAll(); document.getElementById(\'notifDropdown\').remove();" type="button">Clear All</button>';
  }
  headerHtml += '</div>';

  var bodyHtml = '<div class="titlebar-dropdown-body">';
  if (active.length === 0) {
    bodyHtml += '<div class="notif-empty">No notifications.</div>';
  } else {
    for (var i = 0; i < active.length; i++) {
      var n = active[i];
      bodyHtml += '<div class="notif-item" data-id="' + esc(n.id) + '">';
      bodyHtml += '  <span class="notif-item-icon">' + (n.icon || '🔔') + '</span>';
      bodyHtml += '  <div class="notif-item-content">';
      bodyHtml += '    <div class="notif-item-title">' + esc(n.title) + '</div>';
      if (n.body) bodyHtml += '    <div class="notif-item-body">' + esc(n.body) + '</div>';
      bodyHtml += '    <div class="notif-item-time">' + _notifTimeAgo(n.timestamp) + '</div>';
      bodyHtml += '  </div>';
      bodyHtml += '  <button class="notif-item-dismiss" onclick="PantheonNotifications.dismiss(\'' + n.id + '\'); this.closest(\'.notif-item\').remove(); if(!document.querySelector(\'.notif-item\')){ var h=document.querySelector(\'.notif-dropdown .titlebar-dropdown-header\'); if(h)h.innerHTML=\'🔔 Notifications\'; var b=document.querySelector(\'.notif-dropdown .titlebar-dropdown-body\'); if(b)b.innerHTML=\'<div class=\\\"notif-empty\\\">No notifications.</div>\'; }" type="button" title="Dismiss">✕</button>';
      bodyHtml += '</div>';
    }
  }
  bodyHtml += '</div>';

  dd.innerHTML = headerHtml + bodyHtml;
  document.body.appendChild(dd);
  const rect = btn.getBoundingClientRect();
  dd.style.top = (rect.bottom + 4) + 'px';
...
function _closeTitlebarDropdown(e) {
  const dd = document.querySelector('.titlebar-dropdown, .notif-dropdown');
...
function toggleHealthPopup() {
  ...
  const dd = document.createElement('div');
  dd.id = 'healthPopup';
  dd.className = 'titlebar-dropdown health-popup';
  dd.innerHTML =
    '<div class="titlebar-dropdown-header">● System Health <span class="health-refresh-btn" onclick="refreshHealthPopup(this)" title="Refresh">🔄</span></div>' +
    '<div class="titlebar-dropdown-body health-popup-body"><div style="padding:12px;color:var(--muted);font-size:12px">Loading...</div></div>';
  document.body.appendChild(dd);
  const rect = btn.getBoundingClientRect();
  dd.style.top = (rect.bottom + 4) + 'px';
  dd.style.right = (window.innerWidth - rect.right) + 'px';
  ...
  refreshHealthPopup(dd.querySelector('.health-refresh-btn'));
}

function refreshHealthPopup(triggerEl) {
  var body = triggerEl ? triggerEl.closest('.health-popup').querySelector('.health-popup-body')
                       : document.querySelector('.health-popup-body');
  if (!body) return;
  body.innerHTML = '<div style="padding:12px;color:var(--muted);font-size:12px">Checking...</div>';
  fetch('/api/system/health')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var h = '';
      // Aggregate dot — check status
      var dotColor = '#51CF66'; // green
      if (data.status === 'partial' || data.status === 'degraded') dotColor = '#FFD43B';
      else if (data.status === 'unavailable' || data.status === 'critical') dotColor = '#FF6B6B';
      var dot = document.getElementById('healthDot');
      if (dot) dot.style.color = dotColor;

      function statusIcon(val) {
        if (!val || val === 'unavailable') return '<span class="health-status health-dead">🔴</span>';
        if (val === 'ok' || val === 'running' || val === 'connected' || val === true) return '<span class="health-status health-ok">🟢</span>';
        if (val === 'active') return '<span class="health-status health-active" title="Actively processing">🟢</span>';
        if (val === 'sleeping') return '<span class="health-status health-sleeping" title="Sleeping">🟡</span>';
        if (val === 'idle') return '<span class="health-status health-idle" title="Idle">🔵</span>';
        if (val === 'dead') return '<span class="health-status health-dead" title="Dead">🔴</span>';
        if (val === 'ready') return '<span class="health-status health-ready" title="Ready">⚪</span>';
        if (val === 'awake') return '<span class="health-status health-active" title="Running">🟢</span>';
        if (val === 'stopped' || val === false) return '<span class="health-status health-dead">⚪</span>';
        return '<span class="health-status health-dead">🔴</span>';
      }
      function escHealth(s) { return esc(String(s == null ? '' : s)); }

      // Hermes / MCP / LiteLLM (from extended payload)
      h += '<div class="health-section">';
      if (data.hermes) {
        var hIcon = statusIcon(data.hermes.status);
        var hDetail = data.hermes.uptime ? 'Running · ' + escHealth(data.hermes.uptime) : (data.hermes.status === 'running' ? 'Running' : escHealth(data.hermes.status));
        h += '<div class="health-row"><span class="health-label">🏛️ Hermes</span>' + hIcon + '<span class="health-detail">' + hDetail + '</span></div>';
      }
      if (data.mcp) {
        var mIcon = statusIcon(data.mcp.status);
        var mDetail = data.mcp.uptime ? 'Connected · ' + escHealth(data.mcp.uptime) : (data.mcp.status === 'connected' ? 'Connected' : escHealth(data.mcp.status));
        h += '<div class="health-row"><span class="health-label">⚡ MCP</span>' + mIcon + '<span class="health-detail">' + mDetail + '</span></div>';
      }
      if (data.litellm) {
        var lIcon = statusIcon(data.litellm.status);
        var lDetail = data.litellm.uptime ? 'Running · ' + escHealth(data.litellm.uptime) : (data.litellm.status === 'running' ? 'Running' : escHealth(data.litellm.status));
        h += '<div class="health-row"><span class="health-label">🤖 LiteLLM</span>' + lIcon + '<span class="health-detail">' + lDetail + '</span></div>';
      }
      h += '</div>';

      // God runtimes (5-state model)
      if (data.gods && data.gods.length) {
        h += '<div class="health-section"><div class="health-section-title">🎭 God Runtimes</div>';
        for (var i = 0; i < data.gods.length; i++) {
          var g = data.gods[i];
          var gIcon = statusIcon(g.status);
          var gStatusDisplay = g.status ? g.status.charAt(0).toUpperCase() + g.status.slice(1) : 'Unknown';
          var gDetail = gStatusDisplay;
          if (g.last_active) {
            gDetail += ' \u00b7 ' + escHealth(g.last_active);
          } else if (g.status === 'active' || g.status === 'awake') {
            gDetail = 'Running';
          }
          var displayName = escHealth(g.display_name || g.name || '');
          h += '<div class="health-row"><span class="health-label">' + (g.icon || '🎭') + ' ' + displayName + '</span>' + gIcon + '<span class="health-detail">' + gDetail + '</span></div>';
        }
        h += '</div>';
      }

      // System resources
      h += '<div class="health-section"><div class="health-section-title">💾 System</div>';
      var mem = data.memory || (data.system && data.system.memory);
      var disk = data.disk || (data.system && data.system.disk);
      if (mem && mem.total_bytes) {
        var memUsed = _fmtBytes(mem.used_bytes);
        var memTotal = _fmtBytes(mem.total_bytes);
        h += '<div class="health-row"><span class="health-label">RAM</span>' + statusIcon('ok') + '<span class="health-detail">' + memUsed + ' / ' + memTotal + ' (' + (mem.percent || '?') + '%)</span></div>';
      } else {
        h += '<div class="health-row"><span class="health-label">RAM</span>' + statusIcon('unavailable') + '<span class="health-detail">N/A</span></div>';
      }
      if (disk && disk.total_bytes) {
        var dUsed = _fmtBytes(disk.used_bytes);
        var dTotal = _fmtBytes(disk.total_bytes);
        h += '<div class="health-row"><span class="health-label">Disk</span>' + statusIcon('ok') + '<span class="health-detail">' + dUsed + ' / ' + dTotal + ' (' + (disk.percent || '?') + '%)</span></div>';
      } else {
        h += '<div class="health-row"><span class="health-label">Disk</span>' + statusIcon('unavailable') + '<span class="health-detail">N/A</span></div>';
      }
      if (data.cpu && data.cpu.percent != null) {
        h += '<div class="health-row"><span class="health-label">CPU</span>' + statusIcon('ok') + '<span class="health-detail">' + data.cpu.percent + '%</span></div>';
      }
      h += '</div>';
    ...
    .catch(function() {
      body.innerHTML = '<div style="padding:12px;color:var(--error);font-size:12px">Failed to load health data.</div>';
    });
}
...
function updateNotifBadge(count) {
  if (!_notifBadgeEl) ensureNotifBadge();
  var btn = document.getElementById('btnNotifications');
  if (!btn || !_notifBadgeEl) return;
  if (count > 0) {
    _notifBadgeEl.textContent = count > 99 ? '99+' : count;
    _notifBadgeEl.style.display = '';
    btn.classList.add('notif-new');
  } else {
...
function toggleNotificationsDropdown() {
  ...
  dd.className = 'titlebar-dropdown notif-dropdown';
  dd.innerHTML =
    '<div class="titlebar-dropdown-header">' +
      '<span>🔔 Notifications</span>' +
      '<button class="notif-clear-all" onclick="clearAllNotifications()" type="button">Mark all read</button>' +
    '</div>' +
    '<div class="titlebar-dropdown-body notif-dropdown-body"><div style="padding:12px;color:var(--muted);font-size:12px">Loading...</div></div>';
  document.body.appendChild(dd);
  var rect = btn.getBoundingClientRect();
  dd.style.top = (rect.bottom + 4) + 'px';
  dd.style.right = (window.innerWidth - rect.right) + 'px';
  dd.style.position = 'fixed';
  refreshNotifications(dd);
  setTimeout(function() {
    document.addEventListener('click', _closeTitlebarDropdown, {once:true});
    setTimeout(function() {
      document.addEventListener('click', _closeTitlebarDropdown, {once:true});
...
function refreshNotifications(dd) {
  dd = dd || document.getElementById('notifDropdown');
  var body = dd ? dd.querySelector('.titlebar-dropdown-body') : null;
  fetch('/api/notifications/poll')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var notifs = data.notifications || [];
      var active = notifs.filter(function(n) { return !n.dismissed; });
      updateNotifBadge(active.length);
      if (body) renderNotificationBody(body, notifs);
    })
    .catch(function() {
      updateNotifBadge(0);
      if (body) body.innerHTML = '<div style="padding:12px;color:var(--error);font-size:12px">Failed to load notifications.</div>';
    });
}

function renderNotificationBody(body, notifs) {
  var active = notifs.filter(function(n) { return !n.dismissed; });
  if (!active.length) {
  ...
  for (var i = 0; i < active.length; i++) {
    var n = active[i];
    var iconMap = {error:'🔴', warning:'🟡', info:'🔔', success:'🟢'};
    var icon = n.icon || iconMap[n.type] || '🔔';
    var timeStr = '';
    if (n.timestamp) {
      var d = new Date(n.timestamp);
      var now = Date.now();
      var diffMs = now - n.timestamp;
    ...
    var expandedLink = '';
    if (n.action === 'popout' || n.body && n.body.length > 120) {
      expandedLink = '<span class="notif-popout-btn" onclick="event.stopPropagation();popOutNotification(\'' + esc(n.id) + '\')" title="Expand">↗</span>';
    }
    html += '<div class="notif-item" data-id="' + esc(n.id) + '" onclick="popOutNotification(\'' + esc(n.id) + '\')">' +
      '<span class="notif-item-icon">' + icon + '</span>' +
      '<div class="notif-item-content">' +
        '<div class="notif-item-title">' + esc(n.title) + '</div>' +
        '<div class="notif-item-body">' + esc((n.body || '').substring(0, 200)) + '</div>' +
        '<div class="notif-item-time">' + timeStr + '</div>' +
      '</div>' +
      expandedLink +
      '<button class="notif-item-dismiss" onclick="event.stopPropagation();dismissNotification(\'' + esc(n.id) + '\')" title="Dismiss">✕</button>' +
    '</div>';
  }
  body.innerHTML = html;
}
...
function dismissNotification(id) {
  ...
  .then(function(data) {
    if (data.ok) {
      ...
      var item = document.querySelector('.notif-item[data-id="' + esc(id) + '"]');
      if (item) {
  ...
  .catch(function(err) {
    ...
    // Fallback: try the generic notifications POST with dismissed flag
    fetch('/api/notifications/poll')
      .then(function(r) { return r.json(); })
      .then(function(data) {
        var notifs = data.notifications || [];
        ...
        for (var i = 0; i < notifs.length; i++) {
          if (notifs[i].id === id && !notifs[i].dismissed) {
            notifs[i].dismissed = true;
            found = true;
            break;
          }
        }
        if (found) {
          // Write back via the existing notifications create endpoint won't work,
          // but we can at least hide it from the UI
          var item = document.querySelector('.notif-item[data-id="' + esc(id) + '"]');
          if (item) { item.style.display = 'none'; setTimeout(function() { item.remove(); }, 300); }
...
function clearAllNotifications() {
  ...
  .then(function(data) {
    if (data.ok) {
      ...
      if (body) body.innerHTML = '<div class="notif-empty">✨ No new notifications</div>';
      updateNotifBadge(0);
      refreshNotifications();
    }
  })
...
function popOutNotification(id) {
  // Find notification data from the DOM or fetch fresh
  var overlay = document.createElement('div');
  overlay.id = 'notifPopupOverlay';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:2000;background:rgba(0,0,0,.7);display:flex;align-items:center;justify-content:center;padding:24px;backdrop-filter:blur(4px)';
  
  var popup = document.createElement('div');
  popup.id = 'notifPopup';
  popup.style.cssText = 'background:var(--bg2,#1a1a2e);border:1px solid var(--border,rgba(255,255,255,.1));border-radius:16px;box-shadow:0 24px 80px rgba(0,0,0,.5);max-width:580px;width:100%;max-height:80vh;overflow-y:auto;padding:24px;position:relative';
  popup.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><span style="font-size:20px;font-weight:700;color:var(--text)">📬 Notification Details</span><button onclick="closeNotifPopup()" style="background:none;border:none;color:var(--muted);font-size:20px;cursor:pointer;padding:4px;border-radius:6px" title="Close">✕</button></div><div id="notifPopupBody" style="color:var(--text);font-size:14px;line-height:1.6"><div style="padding:20px;text-align:center;color:var(--muted)">Loading...</div></div>';
  ...
  fetch('/api/notifications/poll')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var notifs = (data.notifications || []).filter(function(n) { return n.id === id; });
      var n = notifs[0];
      var bodyEl = document.getElementById('notifPopupBody');
      if (!bodyEl) return;
      if (!n) {
        bodyEl.innerHTML = '<div style="text-align:center;color:var(--error)">Notification not found.</div>';
        return;
      }
      var iconMap = {error:'🔴', warning:'🟡', info:'🔔', success:'🟢'};
      var icon = n.icon || iconMap[n.type] || '🔔';
      var d = n.timestamp ? new Date(n.timestamp).toLocaleString() : 'Unknown';
      var typeLabel = n.type || 'info';
      bodyEl.innerHTML =
        '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">' +
          '<span style="font-size:32px">' + icon + '</span>' +
          '<div>' +
            '<div style="font-size:18px;font-weight:700">' + esc(n.title) + '</div>' +
            '<div style="font-size:12px;color:var(--muted);margin-top:2px">' + esc(typeLabel) + ' · ' + esc(d) + '</div>' +
          '</div>' +
        '</div>' +
        '<div style="background:var(--surface,#12121f);border-radius:10px;padding:16px;font-size:14px;line-height:1.7;white-space:pre-wrap;word-break:break-word">' + esc(n.body || '(no details)') + '</div>' +
        '<div style="margin-top:16px;display:flex;gap:8px;justify-content:flex-end">' +
          '<button onclick="closeNotifPopup();dismissNotification(\'' + esc(n.id) + '\')" style="background:var(--hover-bg);border:1px solid var(--border);color:var(--text);padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px">Dismiss</button>' +
          '<button onclick="closeNotifPopup()" style="background:var(--accent);border:none;color:#fff;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600">Close</button>' +
        '</div>';
    })
    .catch(function() {
      var bodyEl = document.getElementById('notifPopupBody');
      if (bodyEl) bodyEl.innerHTML = '<div style="text-align:center;color:var(--error)">Failed to load details.</div>';
    });
}

function closeNotifPopup() {
  var overlay = document.getElementById('notifPopupOverlay');
  if (overlay) overlay.remove();
}
...
function startNotificationPolling() {
  ...
  fetch('/api/notifications/poll')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var active = (data.notifications || []).filter(function(n) { return !n.dismissed; });
      updateNotifBadge(active.length);
    })
    .catch(function() {});
  // Poll every 30s
  ...
  _notifPollTimer = setInterval(function() {
    fetch('/api/notifications/poll')
      ...
      .then(function(data) {
        var active = (data.notifications || []).filter(function(n) { return !n.dismissed; });
        updateNotifBadge(active.length);
        // Update open dropdown body if visible
        var dd = document.getElementById('notifDropdown');
        if (dd) {
          var body = dd.querySelector('.titlebar-dropdown-body');
          if (body) renderNotificationBody(body, data.notifications || []);
        }
      })
      .catch(function() {});
  }, 30000);
...
function subscribeToPush() {
  ...
  // Check current subscription status and get VAPID key
        ...
        navigator.serviceWorker.ready.then(function(reg) {
          reg.pushManager.subscribe({
            ...
            fetch('/api/push/subscribe', {
              ...
              if (data.ok) _pushSubscribed = true;
            }).catch(function() {});
          }).catch(function(err) {
...
// Subscribe for push after a short delay (let app load first)
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
...
/* ── Boon Drawer — Phase 1: toggle open/close ────────────────────────── */
var _boonDrawerReady = false;
function _initBoonDrawer() {
  if (_boonDrawerReady) return;
  var overlay = $('boonOverlay');
  var backdrop = $('boonOverlayBackdrop');
  var closeBtn = $('boonDrawerClose');
  if (!overlay) { console.warn('[boon-drawer] overlay not found'); return; }
  _boonDrawerReady = true;

  function isVisible() { return overlay.style.display === 'flex'; }
  function open()  {
    overlay.style.display = 'flex';
    var btn = $('btnToggleBoons'); if (btn) btn.classList.add('active');
    if (typeof window.loadBoonList === 'function') window.loadBoonList();
  }
  function close() {
    overlay.style.display = 'none';
    var btn = $('btnToggleBoons'); if (btn) btn.classList.remove('active');
  }
  function toggle(){ isVisible() ? close() : open(); }

  if (closeBtn) closeBtn.addEventListener('click', function(e){ e.preventDefault(); close(); });
  if (backdrop) backdrop.addEventListener('click', function(e){ e.preventDefault(); close(); });
  document.addEventListener('keydown', function(e){ if (e.key === 'Escape' && isVisible()) close(); });

  /* Only expose open/close for promoteMsgToBoon — do NOT overwrite window.toggleBoonDrawer */
  window._boonDrawerOpen = open;
  window._boonDrawerClose = close;
}

/* Try immediate init; also retry on DOMContentLoaded as safety net */
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    _initBoonDrawer();
    /* Retry after a short delay in case elements are dynamically added */
    setTimeout(function() { if (!_boonDrawerReady) _initBoonDrawer(); }, 1000);
  });
} else {
  _initBoonDrawer();
  setTimeout(function() { if (!_boonDrawerReady) _initBoonDrawer(); }, 1000);
...
/* ── Promote message to boon ─────────────────────────────────────────── */
function promoteMsgToBoon(btn) {
  var row = btn.closest('[data-raw-text]');
  var text = row ? row.dataset.rawText : '';
  if (!text) { showToast('Nothing to promote'); return; }

  /* Determine type and title from content */
  var firstLine = text.split('\n')[0].trim().replace(/^#+\s*/, '');
  var title = firstLine.slice(0, 120) || 'Boon';
  var type = 'text';
  var format = 'plain';
  if (/^\s*<!DOCTYPE|<html\b/i.test(text) || /<(head|body|div|script|style|table|form|svg)\b/i.test(text)) {
    type = 'html'; format = 'html';
  } else if (/```[\s\S]*```/m.test(text) && text.split('\n').length > 3) {
    type = 'code'; format = 'plain';
  } else if (/^#{1,6}\s|\*\*|\[.*\]\(.*\)/.test(text)) {
    type = 'markdown'; format = 'markdown';
  }

  /* Get source god from active profile */
  var sourceGod = 'hermes';
  try {
    if (typeof S !== 'undefined' && S.activeProfile) {
      sourceGod = S.activeProfile;
    }
    if (window._godMap && window._godMap[sourceGod] && window._godMap[sourceGod].display_name) {
      sourceGod = window._godMap[sourceGod].display_name;
    }
  } catch(e) {}

  api('/api/boons/create', {
    method: 'POST',
    body: JSON.stringify({
      title: title,
      body: text,
      type: type,
      format: format,
      source_god: sourceGod,
      source_communion: (typeof S!=='undefined'&&S.session&&S.session.session_id)||''
    })
  }).then(function(boon) {
    showToast('📜 Boon created: ' + (boon.title || title));
    /* Open drawer to show the new boon */
    if (typeof window._boonDrawerOpen === 'function') {
      window._boonDrawerOpen();
    }
    /* Refresh boon list if function exists */
    if (typeof window.loadBoonList === 'function') {
      window.loadBoonList();
    }
  }).catch(function(err) {
    showToast('Failed to create boon: ' + (err.message || err), 3000, 'error');
  });
}
window.promoteMsgToBoon = promoteMsgToBoon;

/* ── Global boon drawer toggle (called from inline onclick on 📜 button) ── */
window.toggleBoonDrawer = function() {
  var overlay = document.getElementById('boonOverlay');
  var btn = document.getElementById('btnToggleBoons');
  if (!overlay) return;
  var wasClosed = overlay.style.display === 'none';
  overlay.style.display = wasClosed ? 'flex' : 'none';
  if (btn) btn.classList.toggle('active');
  if (wasClosed) { window.loadBoonList(); }
};

/* ── Load boons from API and render cards in drawer ── */
window.loadBoonList = function() {
  var body = document.getElementById('boonDrawerBody');
  var empty = document.getElementById('boonDrawerEmpty');
  if (!body) return;

  /* Save clone before innerHTML destroys it */
  var emptyHTML = empty ? empty.outerHTML : '';

  body.innerHTML = '<div class="boon-card boon-card--loading">Loading boons…</div>';

  fetch('/api/boons/list')
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var boons = data.boons || data;
      if (!boons.length) {
        body.innerHTML = emptyHTML || '<div class="boon-drawer-empty"><div class="boon-drawer-empty-icon">📜</div><div class="boon-drawer-empty-text">No boons yet</div></div>';
        return;
      }
      var html = '';
      boons.forEach(function(b) { html += window._renderBoonCardHTML(b); });
      body.innerHTML = html;
    })
    .catch(function(err) {
      body.innerHTML = '<div class="boon-card boon-card--error">⚠️ Failed to load boons</div>';
      showToast('Boon list load failed: ' + (err.message || 'unknown'), 3000, 'error');
    });
};

/* ── Render a single boon card (returns HTML string) ── */
window._boonTypeColors = { html:'#e67e22', code:'#3498db', markdown:'#2ecc71', text:'#95a5a6' };
window._renderBoonCardHTML = function(boon) {
  var color = window._boonTypeColors[boon.type] || '#95a5a6';
  var shortBody = (boon.body || '').slice(0, 120);
  var timeStr = '';
  try { var d = new Date(boon.timestamp); timeStr = d.toLocaleDateString(undefined,{month:'short',day:'numeric'}) + ' ' + d.toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit'}); } catch(e){}
  return '<div class="boon-card" data-boon-id="' + boon.id + '" onclick="window._openBoonById(\'' + boon.id + '\')">' +
    '<div class="boon-card-header">' +
      '<span class="boon-card-title">' + _escHtml(boon.title || 'Untitled') + '</span>' +
      '<span class="boon-card-type" style="background:' + color + '20;color:' + color + '">' + _escHtml(boon.type) + '</span>' +
    '</div>' +
    '<div class="boon-card-meta">' +
      '<span class="boon-card-god">' + _escHtml(boon.source_god || '') + '</span>' +
      (timeStr ? '<span class="boon-card-time">' + timeStr + '</span>' : '') +
    '</div>' +
    (shortBody ? '<div class="boon-card-preview">' + _escHtml(shortBody) + '</div>' : '') +
  '</div>';
};

/* ── Open a boon by ID (called from card onclick) ── */
window._openBoonById = function(id) {
  console.log('[boon-drawer] open boon:', id);
  showToast('📜 Opening boon (Phase 3)', 2000);
};
...
function openIdeaEditor(sectionName, entryName, currentStatus, currentNotes, bodyEl) {
  // Close any existing overlay
  var existing = document.querySelector('.ideas-edit-overlay');
  if (existing) existing.remove();

  // Collect all available sections from the rendered dropdown
  var sections = [];
  document.querySelectorAll('.ideas-section').forEach(function(s) {
    var name = s.getAttribute('data-section') || s.querySelector('.ideas-section-name').textContent;
    sections.push(name);
  });
  if (sections.indexOf(sectionName) === -1) sections.push(sectionName);
  ...
  for (var i = 0; i < sections.length; i++) {
    secOpts += '<option value="' + _escHtml(sections[i]) + '"' + (sections[i] === sectionName ? ' selected' : '') + '>' + _escHtml(sections[i]) + '</option>';
  }
  // Status options
  var statOpts = '';
  for (var i = 0; i < statuses.length; i++) {
    statOpts += '<option value="' + statuses[i] + '"' + (statuses[i] === currentStatus ? ' selected' : '') + '>' + statuses[i] + '</option>';
  }

  var overlay = document.createElement('div');
  overlay.className = 'ideas-edit-overlay';
  overlay.id = 'ideasEditOverlay';
  overlay._oSection = sectionName;
  overlay._oName = entryName;
  overlay._bodyEl = bodyEl;
  // Backdrop click closes — pure inline, no function call
  overlay.setAttribute('onclick', 'if(event.target===this)this.remove();');

  overlay.innerHTML =
    '<div class="ideas-edit-card" onclick="event.stopPropagation()">' +
      '<div class="ideas-edit-header">' +
        '<span>Edit Idea</span>' +
        '<button class="ideas-edit-close" onclick="var o=document.getElementById(\'ideasEditOverlay\');if(o)o.remove();" type="button">\u2715</button>' +
      '</div>' +
      '<div class="ideas-edit-body">' +
        '<div class="ideas-edit-row">' +
          '<label class="ideas-edit-label">Name</label>' +
          '<input class="ideas-edit-input" id="ideaEditName" value="' + _escHtml(entryName) + '" />' +
        '</div>' +
        '<div class="ideas-edit-row">' +
          '<label class="ideas-edit-label">Section</label>' +
          '<select class="ideas-edit-select" id="ideaEditSection">' + secOpts + '</select>' +
        '</div>' +
        '<div class="ideas-edit-row">' +
          '<label class="ideas-edit-label">Status</label>' +
          '<select class="ideas-edit-select" id="ideaEditStatus">' + statOpts + '</select>' +
        '</div>' +
        '<div class="ideas-edit-row">' +
          '<label class="ideas-edit-label">Notes</label>' +
          '<textarea class="ideas-edit-textarea" id="ideaEditNotes" rows="3">' + _escHtml(currentNotes) + '</textarea>' +
        '</div>' +
      '</div>' +
      '<div class="ideas-edit-footer">' +
        '<button class="ideas-edit-cancel" onclick="var o=document.getElementById(\'ideasEditOverlay\');if(o)o.remove();" type="button">Cancel</button>' +
        '<button class="ideas-edit-save" id="ideaEditSaveBtn" onclick="_ideasSave()" type="button">Save</button>' +
      '</div>' +
    '</div>';
  document.body.appendChild(overlay);

  setTimeout(function() { document.getElementById('ideaEditName').focus(); }, 100);
}

window._ideasSave = function() {
  var overlay = document.getElementById('ideasEditOverlay');
  if (!overlay) return;
  var origSection = overlay._oSection;
  var origName = overlay._oName;
  var bodyEl = overlay._bodyEl;
  var newName = document.getElementById('ideaEditName').value.trim();
  if (!newName) { document.getElementById('ideaEditName').focus(); return; }
  var newSection = document.getElementById('ideaEditSection').value;
  var newStatus = document.getElementById('ideaEditStatus').value;
  var newNotes = document.getElementById('ideaEditNotes').value.trim();

  var saveBtn = document.getElementById('ideaEditSaveBtn');
  ...
  .then(function(resp) {
    ...
    if (resp.error) { alert('Edit failed: ' + resp.error); return; }
    overlay.remove();
    if (bodyEl && resp.sections) {
      renderIdeasBody(bodyEl, resp);
      var sel = document.getElementById('ideasAddSection');
      if (sel && resp.sections) {
        sel.innerHTML = '';
        resp.sections.forEach(function(s) {
          var opt = document.createElement('option');
          opt.value = s.name;
