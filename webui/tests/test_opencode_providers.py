...
def test_live_models_handler_delegates_to_provider_model_ids():
    ...
    # The old per-provider base_url hardcoding should be gone from _handle_live_models.
    # The function body is extracted because R602-D (delegation chat fallback)
    # legitimately hardcodes a fallback URL in _chat_completion_reply, and that
    # is a separate concern from live model fetching.
    import re as _re2
    func_match = _re2.search(
        r"def _handle_live_models\(.*?(?=\n    # ──|$)",
        routes_src,
        _re2.DOTALL,
    )
    func_body = func_match.group(0) if func_match else routes_src
    assert "https://api.openai.com/v1" not in func_body, (
